---
name: dlake-txdownloaderpro-sage50ca-dynamiccrm
description: >-
  What the shipped default TxDownloaderPro templates set up when Dynamics CRM is the writeback
  destination and SAGE 50 CA is the source: the 11 default templates the catalogue ships for
...
# TxDownloaderPro ← Dynamics CRM (SAGE 50 CA): what the shipped default templates set up
...
|---|---|---|---|---|
| Create New Invoice | MS Dynamic Opportunity to Sage 50 CA Invoice Import | `Opportunity`, `Quote`, `Order` → `Invoice` | 3 | create |
| Create New Quote | MS Dynamic Opportunity to Sage 50 CA Quote Import | `Opportunity`, `Order`, `Quote` → `Quote` | 3 | create |
| Create New SalesOrder | MS Dynamic Order to Sage 50 CA Order Import | `Order`, `Quote`, `Opportunity` → `Order` | 3 | create |
...
- **Objects read:** `account`, `contact`, `Opportunity`, `Opportunitydetail`, `quote`,
  `quotedetail`, `order`, `orderdetail`, `opportunity`, `opportunitydetail`.
- **Child collections pulled in the same query:** `contact`, `Opportunitydetail`, `account`,
...
- **FetchXML elements used:** `fetch`, `entity`, `attribute`, `link-entity`, `filter`,
  `condition`, `order`, `opportunity`, `quote`; condition operators: `null`, `not-null`, `eq`.
...
- **Template path roots used:** `account`, `opportunity`, `quote`, `order`, `opportunitydetail`,
  `quotedetail`, `orderdetail`. A path’s first segment has to match the element the engine
...
system’s response after the write (parent §11). Of this pair’s 11 default templates, 2 carry a
