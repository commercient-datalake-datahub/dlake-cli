---
name: dlake-crmpro-deltekajera-hubspot
description: >-
  What the shipped CRMPro templates set up for a DELTEK AJERA source pushing into HubSpot: the
  ...
  outcome of each template group — create contact. Use it when standing up or reading a DELTEK
  AJERA → HubSpot template set, when deciding which templates to import and activate, or when a
...
# CRMPro → HubSpot (DELTEK AJERA): what the shipped templates set up
...
for this CRM; read it alongside this one. What follows is only what the shipped DELTEK AJERA →
...
|---|---|---|---|
| **create contact** | ERP `SF_ERP_Salesforce_Clone_BillingContacts`, `SF_ERP_Salesforce_Clone_Customer` data becomes `contact` in HubSpot. New records are created and existing ones updated; none are deleted. | `contact` | `SF_ERP_Salesforce_Clone_BillingContacts`, `ctedistinctemails`, `SF_ERP_Salesforce_Clone_Customer` |
...
|---|---|---|---|---|---|
| `vw_HUBSPOT_NEW_CONTACT` | insert-only | `HUBSPOT_NEW_CONTACT::` | `HUBSPOT_NEW_CUSTOMER::` | `RecordKey`, `externalkey` | `SF_ERP_Salesforce_Clone_BillingContacts`, `ctedistinctemails`, `SF_ERP_Salesforce_Clone_Customer` |
...
|---|---|---|---|
| create contact | `contact` | 20 | `ContactKey` → `externalkey`, `FirstName` → `firstname`, `Title` → `title`, `LastName` → `lastname`, `FaxNumber` → `fax_number` |
...
adds what the shipped DELTEK AJERA → HubSpot templates set up. `dlake-crmpro-hubspot` is the
