---
name: dlake-crmpro-sage100us-salesforce
description: >-
  What the shipped CRMPro templates set up for a SAGE100US source pushing into Salesforce: the
  `CRM_Configuration` row each template inserts (object API names `users`, `Account`,
  `CommercientSF8__AR_TRANSACTIONPAYMENTHISTORY__c`, `CommercientSF8__SAGE100_TermsCode__c`,
  ...
  USER, Account, AR Invoice Payments, CRM Opportunity and Line, CRM Ownership, CRM Quote and Line,
  Customer Multi Ship Addresses, Product and more. Use it when standing up or reading a SAGE100US
...
# CRMPro → Salesforce (SAGE100US): what the shipped templates set up
...
shipped SAGE100US → Salesforce templates themselves set, read from their `Insert_Query`,
...
adds what the shipped SAGE100US → Salesforce templates set up. `dlake-crmpro-syspro-salesforce` is
