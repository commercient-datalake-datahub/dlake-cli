---
name: dlake-txdownloaderpro-msdynamicgp-dynamiccrm
description: >-
  What the shipped default TxDownloaderPro templates set up when Dynamics CRM is the writeback
  destination and MSDynamicGP is the source: the 1 default template the catalogue ships for this
...
# TxDownloaderPro ← Dynamics CRM (MSDynamicGP): what the shipped default templates set up
...
here exactly as the rows spell it — `MSDynamicGP` — and not by a display name.
...
|---|---|---|---|---|
| Create New Customer | Dynamic Account to Dynamic GP Customer | `Account` → `Customer` | 1 | create |
