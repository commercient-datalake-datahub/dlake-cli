---
name: dlake-crmpro-datacorchempax-hubspot
description: >-
  What the shipped CRMPro templates set up for a DATACOR-CHEMPAX source pushing into HubSpot: the
  ...
  outcome of each template group — create contact. Use it when standing up or reading a
  DATACOR-CHEMPAX → HubSpot template set, when deciding which templates to import and activate, or
...
# CRMPro → HubSpot (DATACOR-CHEMPAX): what the shipped templates set up
...
for this CRM; read it alongside this one. What follows is only what the shipped DATACOR-CHEMPAX →
...
|---|---|---|---|
| **create contact** | ERP `SF_ERP_Salesforce_Clone_CUSTFIL2` data becomes `contact` in HubSpot. New records are created and existing ones updated; none are deleted. | `contact` | `SF_ERP_Salesforce_Clone_CUSTFIL2` |
...
|---|---|---|---|---|---|---|
| `vw_HUBSPOT_NEW_CONTACT` | insert-only | `HUBSPOT_NEW_CONTACT::` | `HUBSPOT_NEW_CUSTOMER::` | `RecordKey`, `externalkey` | yes | `SF_ERP_Salesforce_Clone_CUSTFIL2` |
...
adds what the shipped DATACOR-CHEMPAX → HubSpot templates set up. `dlake-crmpro-hubspot` is the
