...
description: >-
  ...
  destination and Sage 50 US is the source: the 13 default templates the catalogue ships for
...
`dlake-txdownloaderpro-dynamicscrm/erps/sage-50-us.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New SalesInvoice | MS Dynamic Opportunity to Sage 50 US Sales Invoice | `Opportunity`, `Quote`, `Order` → `Invoice` | 4 | create |
| Create New SalesOrder | Dynamic Quote to Sage 50 Sales Order | `Quote`, `Order`, `Opportunity` → `Order` | 3 | create |
| Update SalesOrder | Dynamic Quote to Sage 50 Sales Order - Update Sales Order | `Quote`, `Order`, `Opprtunity` → `Sales Order` | 3 | update |
| Create New Customer | Dynamic CRM Account to Sage 50 US Customer | `account` → `Customer` | 1 | create |
| Delete SalesInvoice | Salesforce Opportunity to Sage 50 US Invoice - Delete Invoice | — → — | 1 | delete |
| Delete SalesOrder | Salesforce Opportunity to Sage 50 US Sales Order - Delete Sales Order | — → — | 1 | delete |

Across the 13 default templates: 8 carry `IsInsert`, 3 carry `IsUpdate`, 2 carry `IsDelete`. A
...
`TxDownloader_1_3`, `TxDownloader_1_2`, `TxDownloader_1_8`, `TxDownloader_1_7`,
`TxDownloader_1_5`.
...
- **Objects read:** `account`, `contact`, `quote`, `quotedetail`, `salesorder`,
  `salesorderdetail`, `order`, `orderdetail`, `opportunity`, `opportunitydetail`, `Opportunity`.
- **Child collections pulled in the same query:** `contact`, `account`, `quotedetail`,
  `salesorderdetail`, `orderdetail`, `opportunitydetail`. A header retrieved without its lines
  is a query that does not name the child collection.
- **FetchXML elements used:** `fetch`, `entity`, `attribute`, `link-entity`, `filter`,
  `condition`, `order`; condition operators: `null`, `not-null`, `eq`.
...
- **Template path roots used:** `opportunity`, `quote`, `order`, `account`, `opportunitydetail`,
  `quotedetail`, `orderdetail`, `salesorderdetail`, `invoicedetail`, `salesorder`, `invoice`,
...
- **`Line.` section members present:** `Line.Quantity`, `Line.Unit_Price`, `Line.Tax_Type`,
  `Line.Item_ID`, `Line.mainXml`, `Line.GL_Account`, `Line.SO_Description`, `Line.Description`,
  `Line.Amount`, `Line.Stocking_Quantity`. 10 templates name the collection through
...
system’s response after the write (parent §11). Of this pair’s 13 default templates, 1 carries a
...
- **CRM fields `Part1` writes to:** `comrcint_arcustomercode`. These are the fields on the
