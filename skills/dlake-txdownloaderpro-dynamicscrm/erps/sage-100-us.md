...
`dlake-txdownloaderpro-dynamicscrm/erps/sage-100-us.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Sales Invoice | MS Dynamic Invoice To Sage100 Invoice | `Invoice`, `Order`, `Quote` → `Invoice` | 4 | create |
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemCode`, `Line.UnitPrice`,
  ...
  `Line.Quantityopportunityed`, `Line.Quantityordered`. 7 templates name the collection through
  `Line.mainXml`; the members beside it are resolved against that collection’s own root rather
  than through the header.
- **`$FUN_` value tokens the documents carry:** `$FUN_STRREPLACE`, `$FUN_UNESCAPEXML`. The names
...
- **CRM fields `Part1` writes to:** `comrcint_arcustomercode`, `Sage_Invoice_Number__c`,
  `Sage_order_Number__c`, `Sage_quote_Number__c`, `Sage_opportunity_Number__c`. These are the
...
- **Response fields it reads them from:** `CustomerNo`, `InvoiceNo`, `orderNo`, `quoteNo`,
  `opportunityNo`. The map is written **source-path first, CRM-field second** (parent §11); the
...
- **Operations:** 8 carry `IsInsert`, 0 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Customer` (3), `Create New Sales
  Invoice` (3), `Create New Customer company 6` (1), `Create New Invoice company 6` (1).
- **Field-process versions they belong to:** `TxDownloader_2_1`, `TxDownloader_2_3`.
