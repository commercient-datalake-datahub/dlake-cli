---
name: dlake-txdownloaderpro-dynamicscrm/erps/myob-accountright
description: >-
  What the shipped default TxDownloaderPro templates set up when Dynamics CRM is the writeback
  destination and MYOB AccountRight is the source: the 2 default templates the catalogue ships
...
# TxDownloaderPro ← Dynamics CRM — MYOB AccountRight: what the shipped default templates set up
...
`dlake-txdownloaderpro-dynamicscrm/erps/myob-accountright.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create Customer | — | `Account` → `Customer` | 2 | create / update |
...
- **Objects read:** `account`, `comrcint_sapb1_salesperson`.
- **Child collections pulled in the same query:** `comrcint_sapb1_salesperson`. A header
...
pair’s 2 default templates, 2 carry a `DefaultProcessStructure`; 2 of those do not parse as JSON
and are counted but not described.
