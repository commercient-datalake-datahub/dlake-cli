...
# TxDownloaderPro ← Dynamics CRM — Sage 50 UK: what the shipped default templates set up
...
`dlake-txdownloaderpro-dynamicscrm/erps/sage-50-uk.md`, and that skill’s ERP table is what
points at it.
...
- **Marker and key columns the queries name:** `Commercient_Import__c`,
  ...
  `CommercientSF__ExternalKey__c`, `Commercient_Update__c`. These are the columns a user’s flag
...
- **`Line.` section members present:** `Line.QTY_ORDER`, `Line.mainXml`, `Line.TAX_CODE`,
  `Line.DESCRIPTION`, `Line.STOCK_CODE`, `Line.UNIT_PRICE`, `Line.IsSearchSTOCK_CODE`,
...
- **`$FUN_` value tokens the documents carry:** `$FUN_STRREPLACE`, `$FUN_SUBSTR`. The names are
...
- **CRM fields `Part1` writes to:** `ExternalKey__c`, `Commercient_ExternalKey__c`,
...
- **Response fields it reads them from:** `INVOICE_NUMBER`, `ORDER_NUMBER`, `ACCOUNT_REF`. The
