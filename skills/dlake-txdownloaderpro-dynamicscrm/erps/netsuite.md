---
name: dlake-txdownloaderpro-dynamicscrm/erps/netsuite
description: >-
  What the shipped default TxDownloaderPro templates set up when Dynamics CRM is the writeback
  destination and NetSuite is the source: the 2 default templates the catalogue ships for this
...
# TxDownloaderPro ← Dynamics CRM — NetSuite: what the shipped default templates set up
...
`dlake-txdownloaderpro-dynamicscrm/erps/netsuite.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Customer | Dynamics CRM Account to NetSuite Customer | `Account` → `Customer` | 2 | create / update |
...
- **FetchXML elements used:** `fetch`, `entity`, `attribute`, `link-entity`, `filter`,
  `condition`; condition operators: `null`, `notnull`.
