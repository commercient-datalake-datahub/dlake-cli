...
# TxDownloaderPro ← Dynamics CRM — Epicor 10: what the shipped default templates set up
...
`dlake-txdownloaderpro-dynamicscrm/erps/epicor-10.md`, and that skill’s ERP table is what points
at it.
...
- **Marker and key columns the queries name:** `Commercient_Import__c`, `Commercient_Update__c`,
  `Commercient_ExternalKey__c`, `CommercientSF__Commercient_ArCustomerCode__c`,
  `Commercient_Company_Name`, `Commercient_Customer_ID`, `Commercient_CustNum`,
  ...
  at all.
- **Operators present:** `=`, `AND`, an empty-string test, `!=`, `OR`. The parent’s §12 is the
...
- **CRM fields `Part1` writes to:** `Commercient_ExternalKey__c`,
  `CommercientSF__Commercient_ArCustomerCode__c`, `ExternalKey__c`. These are the fields on the
...
- **Response fields it reads them from:** `Company`, `CustID`, `QuoteNum`. The map is written
...
- **How many:** 19 community templates, across 6 field-process versions.
- **Operations:** 10 carry `IsInsert`, 9 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Account` (1), `Create New Contact` (1),
  `Create New Contact CANADA` (1), `Create New Contact US` (1), `Create New Customer CANADA`
  (1), `Create New Customer US` (1), `Create New Sales Order CANADA` (1), `Create New Sales
  Order US` (1), `Create New ShipTo CANADA` (1), `Create New ShipTo US` (1), `Update Contact`
  (1), `Update Contact US` (1), `Update Customer` (1), `Update Customer CANADA` (1), `Update
  Customer US` (1), `Update Quote US` (1), `Update Sales Order CANADA` (1), `Update Sales Order
  US` (1), `Update Task` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_19_11`, `TxDownloader_19_2`,
  `TxDownloader_19_1`, `TxDownloader_19_5`, `TxDownloader_19_7`, `TxDownloader_19_6`.
