---
name: dlake-txdownloaderpro-dynamicscrm/erps/quickbooks-desktop
description: >-
  What the shipped default TxDownloaderPro templates set up when Dynamics CRM is the writeback
  destination and QuickBooks Desktop is the source: the 1 default template the catalogue ships
...
# TxDownloaderPro ← Dynamics CRM — QuickBooks Desktop: what the shipped default templates set up
...
`dlake-txdownloaderpro-dynamicscrm/erps/quickbooks-desktop.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Customer Import | Dynamic CRM Account to QuickBooks Customer | `Account` → `Customer` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_5_1`.
...
- **Child collections pulled in the same query:** `contact`. A header retrieved without its
...
- **Template path roots used:** `account`, `Account`. A path’s first segment has to match the
