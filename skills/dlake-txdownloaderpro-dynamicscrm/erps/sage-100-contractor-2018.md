---
name: dlake-txdownloaderpro-dynamicscrm/erps/sage-100-contractor-2018
description: >-
  What the shipped default TxDownloaderPro templates set up when Dynamics CRM is the writeback
  destination and Sage 100 Contractor 2018 is the source: the 4 default templates the catalogue
...
# TxDownloaderPro ← Dynamics CRM — Sage 100 Contractor 2018: what the shipped default templates set up
...
`dlake-txdownloaderpro-dynamicscrm/erps/sage-100-contractor-2018.md`, and that skill’s ERP table
...
|---|---|---|---|---|
| Create/Update Customer | Dynamics CRM Account to Sage100 Contractor Customer | `Account` → `Customer` | 2 | create / update |
| Create/Update Job | — | `Opportunity` → `Job` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_115_1`,
`TxDownloaderPro_115_3`.
...
- **Child collections pulled in the same query:** `contact`, `opportunitydetail`, `account`. A
...
- **FetchXML elements used:** `fetch`, `entity`, `attribute`, `link-entity`, `filter`,
  `condition`, `opportunity`; condition operators: `null`, `notnull`, `not-null`, `eq`.
...
- **Template path roots used:** `account`, `Opportunity`, `opportunity`. A path’s first segment
  has to match the element the engine emits, and the document root itself is never part of the
  path.
- **`Line.` section members present:** `Line.mainXml`. 2 templates name the collection through
...
- **CRM fields `Part1` writes to:** `comrcint_arcustomercode`, `externalkey`. These are the
...
- **Response fields it reads them from:** `ClientId`, `ObjectID`. The map is written
