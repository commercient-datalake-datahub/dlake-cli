---
name: dlake-txdownloaderpro-shopify/erps/netsuite
description: >-
  What the shipped default TxDownloaderPro templates set up when Shopify is the writeback
  destination and NetSuite is the source: the 3 default templates the catalogue ships for this
...
# TxDownloaderPro ← Shopify — NetSuite: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/netsuite.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create New Customer | Shopify Customer to Netsuite Customer | `Customer` → `Customer` | 1 | create |
| Create New Invoice | Shopify Order to Netsuite SalesInvoice | `Order` → `SalesInvoice` | 1 | create |
| Create New SalesOrder | Shopify Order to NETSUITE SalesOrder | `Order` → `SalesOrder` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_9_1`,
`TxDownloader_9_3`, `TxDownloader_9_2`.
...
- **Template path roots used:** `Order`, `Customer`, `line_items`. A path’s first segment has to
  match the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.description`, `Line.quantity`, `Line.price`,
  `Line.mainXml`, `Line.item`. 2 templates name the collection through `Line.mainXml`; the
