...
# TxDownloaderPro ← Shopify — Microsoft Business Central: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/microsoft-business-central.md`, and that skill’s ERP table
...
|---|---|---|---|---|
| Create Customer | Shopify Customer to DYNAMICSBUSINESSCENTRAL Customer | `Customer` → `Customer` | 2 | create |
| Create New Item | Shopify Product to DYNAMICSBUSINESSCENTRAL Product | `Product` → `Product` | 2 | create |
...
- **Template path roots used:** `Order`, `Customer`, `line_items`, `Product`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.unitPrice`, `Line.quantity`,
  `Line.itemId`, `Line.description`, `Line.documentNo`. 4 templates name the collection through
