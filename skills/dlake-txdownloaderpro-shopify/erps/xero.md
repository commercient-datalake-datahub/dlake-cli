---
name: dlake-txdownloaderpro-shopify/erps/xero
description: >-
  What the shipped default TxDownloaderPro templates set up when Shopify is the writeback
  destination and Xero is the source: the 3 default templates the catalogue ships for this pair,
...
# TxDownloaderPro ← Shopify — Xero: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/xero.md`, and that skill’s ERP table is what points at it.
...
|---|---|---|---|---|
| Contact Import | Shopify Customer to XERO Contact | `Customer` → `Customer` | 1 | create |
| Invoice Import | Shopify Order to XERO SalesInvoice | `Order` → `SalesInvoice` | 1 | create |
| Product Items Import | Shopify Product to XERO Product | `Product` → `Product` | 1 | create |
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemCode`, `Line.Description`,
  `Line.Quantity`, `Line.LineAmount`. 1 template name the collection through `Line.mainXml`; the
