---
name: dlake-txdownloaderpro-shopify/erps/quickbooks-desktop
description: >-
...
# TxDownloaderPro ← Shopify — QuickBooks Desktop: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/quickbooks-desktop.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Customer with Contact | Shopify Customer to QuickBooks Customer Contact | `Account` → `Customer Contact` | 1 | create |
| Customer Import | Shopify Customer to QuickBooks Customer | `Customer` → `Customer` | 1 | create |
| Inventory Product Import | — | `Product` → `Product` | 1 | create |
| Sales Invoice Import | Shopify Order to QuickBooks SalesInvoice | `Order` → `SalesInvoice` | 1 | create |
| Sales Order Import | Shopify Order to QuickBooks SalesOrder | `Order` → `SalesOrder` | 1 | create |
...
`TxDownloader_5_1`, `TxDownloader_5_8`, `TxDownloader_5_3`, `TxDownloader_5_2`.
...
- **`Line.` section members present:** `Line.ItemRef`, `Line.Desc`, `Line.Quantity`,
  `Line.Amount`, `Line.mainXml`, `Line.Rate`. 2 templates name the collection through
