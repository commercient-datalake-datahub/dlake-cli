---
name: dlake-txdownloaderpro-shopify/erps/traverse-11
description: >-
...
# TxDownloaderPro ← Shopify — Traverse 11: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/traverse-11.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Customer Import | Shopify Customer to Traverse Customer | `Customer` → `Customer` | 1 | create |
| Sales Invoice Import | Shopify Order to Traverse SalesInvoice | `Order` → `SalesInvoice` | 1 | create |
| SalesOrder Import | Shopify Order to Traverse SalesOrder | `Order` → `SalesOrder` | 1 | create |
...
`TxDownloader_7_3`, `TxDownloader_7_2`.
...
- **`Line.` section members present:** `Line.ItemID`, `Line.Description`, `Line.Qty`,
  `Line.UnitPrice`, `Line.mainXml`. 2 templates name the collection through `Line.mainXml`; the
