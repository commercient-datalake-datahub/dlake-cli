---
name: dlake-txdownloaderpro-shopify/erps/epicor-9-and-9-5
description: >-
...
# TxDownloaderPro ← Shopify — Epicor 9 and 9.5: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/epicor-9-and-9-5.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Customer | Shopify Customer to Epicor 9 Customer | `Customer` → `Customer` | 1 | create |
| SalesOrder | Shopify Order to Epicor 9 SalesOrder | `Order` → `SalesOrder` | 1 | create |
...
- **`Line.` section members present:** `Line.mainXml`, `Line.PartNum`, `Line.UnitPrice`,
  `Line.OrderQty`, `Line.LineDesc`. 1 template name the collection through `Line.mainXml`; the
