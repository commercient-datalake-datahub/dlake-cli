...
description: >-
  ...
  pair, the 11 field-process versions they are identified by, what each template family
  ...
  templates fill for the write back to the CRM. It also lists the 9 community templates the
...
`dlake-txdownloaderpro-shopify/erps/syspro-6.md`, and that skill’s ERP table is what points at
it.
...
Across the 11 default templates: 5 carry `IsInsert`, 3 carry `IsUpdate`, 3 carry `IsDelete`, and
...
`TxDownloaderPro_6_3`, `TxDownloaderPro_6_2`, `TxDownloaderPro_6_11`, `TxDownloaderPro_6_16`,
`TxDownloaderPro_6_12`, `TxDownloaderPro_6_15`.
...
`Query` does not have one shape across the product (parent §9). For this pair, 11 carry a JSON
...
- **Members present in the JSON query object:** `ModuleName` (11). Where a `Where` member is
  present it is empty.
...
- **`Line.` section members present:** `Line.LineActionType`, `Line.StockCode`,
  `Line.StockDescription`, `Line.OrderQty`, `Line.Price`, `Line.LineAlwaysUsePriceEntered`,
  `Line.LineAllocationAction`, `Line.mainXml`, `Line.IsStockLineCommentSameFormat`,
  `Line.IsStockLineCommentRequiredInSequence`. 1 template name the collection through
...
- **Operations:** 9 carry `IsInsert`, 0 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create Customer` (2), `Create New Customer` (2),
  `Create New Sales Order` (2), `Create Order` (1), `Create Payment` (1), `Create SalesOrder`
  (1).
- **Field-process versions they belong to:** `TxDownloaderPro_6_8`, `TxDownloaderPro_6_1`,
  `TxDownloaderPro_6_2`.
