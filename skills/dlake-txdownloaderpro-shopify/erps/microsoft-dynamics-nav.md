...
`dlake-txdownloaderpro-shopify/erps/microsoft-dynamics-nav.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create Customer | Shopify Customer to MSDYNAMICNAV Customer | `Customer` → `Customer` | 1 | create |
| Create SalesOrder | Shopify Order to MSDYNAMICNAV Order | `Order` → `SalesOrder` | 1 | create |
...
- **`Line.` section members present:** `Line.Variant_Code`, `Line.Description`,
  `Line.Qty_To_Assemble_To_Order`, `Line.Unit_Price`, `Line.mainXml`. 1 template name the
