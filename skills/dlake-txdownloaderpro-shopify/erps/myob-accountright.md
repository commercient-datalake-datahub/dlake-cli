---
name: dlake-txdownloaderpro-shopify/erps/myob-accountright
description: >-
  What the shipped default TxDownloaderPro templates set up when Shopify is the writeback
  destination and MYOB AccountRight is the source: the 5 default templates the catalogue ships
...
# TxDownloaderPro ← Shopify — MYOB AccountRight: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/myob-accountright.md`, and that skill’s ERP table is what
points at it.
...
| Create Customer Location/Address | Shopify Customer to MYOBACCOUNTRIGHT Customer Location | `Customer` → `Location` | 1 | create |
...
`TxDownloaderPro_36_7`, `TxDownloaderPro_36_6`, `TxDownloaderPro_36_3`, `TxDownloaderPro_36_4`.
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemUID`, `Line.Description`,
  `Line.UnitPrice`, `Line.ShipQuantity`. 2 templates name the collection through `Line.mainXml`;
