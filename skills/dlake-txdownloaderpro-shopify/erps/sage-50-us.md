---
name: dlake-txdownloaderpro-shopify/erps/sage-50-us
description: >-
  What the shipped default TxDownloaderPro templates set up when Shopify is the writeback
  destination and Sage 50 US is the source: the 4 default templates the catalogue ships for this
...
# TxDownloaderPro ← Shopify — Sage 50 US: what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/sage-50-us.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create New Customer | Shopify Customer to Sage 50 US Customer | `Customer` → `Customer` | 1 | create |
| Create New Product | Shopify Product to Sage 50 US Product | `Product` → `Product` | 1 | create |
| Create New SalesInvoice | Shopify Order to Sage 50 US SalesInvoice | `Order` → `SalesInvoice` | 1 | create |
| Create New SalesOrder | Shopify SalesOrder to Sage 50 US SalesOrder | `Order` → `SalesOrder` | 1 | create |
...
`TxDownloader_1_19`, `TxDownloader_1_3`, `TxDownloader_1_2`.
...
- **`Line.` section members present:** `Line.Quantity`, `Line.GL_Account`, `Line.Unit_Price`,
  `Line.Tax_Type`, `Line.Item_ID`, `Line.Description`, `Line.Amount`, `Line.Stocking_Quantity`,
  `Line.SO_Description`, `Line.mainXml`. 1 template name the collection through `Line.mainXml`;
