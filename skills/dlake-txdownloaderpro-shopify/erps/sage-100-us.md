---
name: dlake-txdownloaderpro-shopify/erps/sage-100-us
description: >-
  What the shipped default TxDownloaderPro templates set up when Shopify is the writeback
  destination and Sage 100 (US) is the source: the 4 default templates the catalogue ships for
...
# TxDownloaderPro ← Shopify — Sage 100 (US): what the shipped default templates set up
...
`dlake-txdownloaderpro-shopify/erps/sage-100-us.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Customer | Shopify Customer to Sage 100 Customer | `Customer` → `Customer` | 1 | create |
| Create new Product | Shopify Product to Sage 100 Product | `Product` → `Product` | 1 | create |
| Create New Sales Invoice | Shopify Order to Sage 100 SalesInvoice | `Order` → `SalesInvoice` | 1 | create |
| Create New Sales Order | Shopify Order to Sage 100 SalesOrder | `Order` → `SalesOrder` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_2_1`,
`TxDownloader_2_7`, `TxDownloader_2_3`, `TxDownloader_2_2`.
...
- **Template path roots used:** `Order`, `Customer`, `line_items`, `Product`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemCode`, `Line.QuantityOrdered`,
  `Line.UnitPrice`, `Line.ItemCodeDesc`, `Line.CommentText`. 2 templates name the collection
...
- **Template names the catalogue carries:** `Create Customer` (1), `Create New Sales Order` (1),
  `Create SalesOrder` (1).
- **Field-process versions they belong to:** `TxDownloader_2_1`, `TxDownloader_2_2`.
