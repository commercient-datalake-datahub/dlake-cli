...
| [`dlake-txdownloaderpro-dynamiccrm/`](dlake-txdownloaderpro-dynamiccrm/SKILL.md) | The **Dynamics CRM** specifics for a TxDownloaderPro writeback, from the shipped default templates: the query shape that finds flagged records and the marker columns it reads, the structure of the inbound mapping document and the `$FUN_` token names it carries, which `ResultStructure` parts the templates fill for the write back to Dynamics CRM, and the process row each template becomes on import. Extends `dlake-txdownloaderpro/`. |
...
| [`dlake-crmpro-syspro-salesforce/`](dlake-crmpro-syspro-salesforce/SKILL.md) | The **Salesforce** specifics for a CRMPro forward sync from a SYSPRO source: the configuration values the Salesforce engine dispatches on — standard objects alongside the managed package's custom objects, the namespace prefix/postfix pair, and an external-id field as the match key — the view contract the shipped templates use (a single-colon repository key, the identity column named as the external id, no `SFDCID` output column), the id-chaining ladder that makes `Sync_Order` a dependency order, the reverse-lookup and create/update process pairs, and what to check when a run completes without pushing any records. Extends `dlake-crmpro/`. |
| [`dlake-crmpro-syspro-shopify/`](dlake-crmpro-syspro-shopify/SKILL.md) | The **Shopify** specifics for a CRMPro forward sync from a SYSPRO source: the upper-case object tokens, the display name that reads as an operation, the insert-only create legs and their `::` repository key, and the two update legs — which are not cursor-driven at all but compare the ERP value against a mirrored Shopify value and push only the difference. Covers why a created product never updates, and what an empty mirror table does to a run. Extends `dlake-crmpro/`. |
...
`dlake-crmpro-syspro-salesforce/` and `dlake-crmpro-syspro-shopify/` — carry the per-destination
values, which genuinely differ. The writeback leg has the same shape: `dlake-txdownloaderpro-salesforce/`,
`-hubspot/`, `-dynamiccrm/`, `-zohocrm/` and `-shopify/` carry the per-destination values for TxDownloaderPro.
...
cp -r skills/dlake-crmpro-syspro-salesforce ~/.claude/skills/   # Salesforce specifics for CRMPro (SYSPRO source)
cp -r skills/dlake-crmpro-syspro-shopify    ~/.claude/skills/   # Shopify specifics for CRMPro (SYSPRO source)
...
cp -r skills/dlake-txdownloaderpro-dynamiccrm ~/.claude/skills/   # Dynamics CRM specifics for TxDownloaderPro
