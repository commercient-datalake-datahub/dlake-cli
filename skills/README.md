...
| [`making-erp-crm-skill-pages/`](making-erp-crm-skill-pages/SKILL.md) | An **authoring** skill, not an operating one: how to produce a `dlake-crmpro-<erp>-<crm>` destination sub-skill for a new ERP × CRM pair from the shipped template catalogue — which data to pull, how to run the generator, which sections a person must write, the naming rule, the publication rules and the review checklist. |
...
values, which genuinely differ; `making-erp-crm-skill-pages/` is how a new pair gets one.
