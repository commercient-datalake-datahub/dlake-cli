...
9. **Registration and passwords are the user's to enter.** `dlake register start` creates an account
   ...
   2025 Express container dedicated to the tenant, provisioned at seeding. Every registration is
   `express` by default, and the choice is locked once the tenant is seeded (`hosting_locked`).
