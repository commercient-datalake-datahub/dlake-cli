...
description: >-
  ...
  no Standard template ships for this pair, and the 21 community templates it does carry are
  ...
  destination objects they write are `account`, `comrcint_ar_customer`, `comrcint_ar_salesperson`,
  `comrcint_ar_terms_code`, `comrcint_invoiceheader` and more. Use it when deciding whether a
...
`dlake-crmpro-dynamicscrm/erps/epicor-eagle.md`, and that skill's ERP table is what points at it.
...
- Destination objects: `account`, `comrcint_ar_customer`, `comrcint_ar_salesperson`, `comrcint_ar_terms_code`, `comrcint_invoiceheader`, `comrcint_invoiceline`, `comrcint_salesorderheader`, `comrcint_salesorderline`, `invoice`, `invoicedetail`, `new_class`, `new_department`, `new_job`, `pricelevel`, `product`, `salesorder`, `salesorderdetail`, `uom`, `uomschedule` and 2 custom objects
- Object display names: CRM Account, CRM Class, CRM Department, CRM Inventory, CRM invoice, CRM invoicedetail, CRM PriceBook, CRM Product, CRM salesorder, CRM salesorderdetail, CRM UOM, CRM UOMSchedule and 9 more
...
adds what the Epicor Eagle → Dynamics CRM template catalogue carries. `dlake-crmpro-dynamicscrm`
