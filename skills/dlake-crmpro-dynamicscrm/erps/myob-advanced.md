---
name: dlake-crmpro-dynamicscrm/erps/myob-advanced
description: >-
  What the CRMPro template catalogue carries for a MYOB Advanced source pushing into Dynamics CRM:
  no Standard template ships for this pair, and the 8 community templates it does carry are stated
  ...
  they write are `account`, `contact`, `transactioncurrency`. Use it when deciding whether a
  shipped template set exists for a MYOB Advanced → Dynamics CRM before standing one up, and what
...
# CRMPro → Dynamics CRM — MYOB Advanced: what the template catalogue carries
...
`dlake-crmpro-dynamicscrm/erps/myob-advanced.md`, and that skill's ERP table is what points at it.
...
- Destination objects: `account`, `contact`, `transactioncurrency`
- Object display names: CRM Accounts, CRM Contacts, CRM Parent Accounts and a further template
...
adds what the MYOB Advanced → Dynamics CRM template catalogue carries. `dlake-crmpro-dynamicscrm`
