...
description: >-
  ...
  destination objects they write are `invoice`, `invoicedetail`, `account`, `product`. Use it when
  deciding whether a shipped template set exists for a QuickBooks Online → Dynamics CRM before
...
`dlake-crmpro-dynamicscrm/erps/quickbooks-online.md`, and that skill's ERP table is what points at
...
- Object display names: Account, CRM Invoice, CRM Invoiceline, Invoice, Invoice Line, Product, QuickBook Online Customer, QuickBook Online Invoice, QuickBook Online InvoiceLine, QuickBook Online Payment, QuickBook Online PaymentLine
...
adds what the QuickBooks Online → Dynamics CRM template catalogue carries.
