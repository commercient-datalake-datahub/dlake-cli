---
name: dlake-txdownloaderpro-hubspot/erps/sage-50-canada
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Sage 50 Canada is the source: the 10 default templates the catalogue ships for
...
# TxDownloaderPro ← HubSpot — Sage 50 Canada: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/sage-50-canada.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Customer | — | `companies` → `Customer` | 2 | create / update |
| Create New ShipToAddress | — | `companies` → `ShipToAddress` | 2 | create / update |
| Create New Contact | — | `contacts` → `Contact` | 1 | create |
| Create New Invoice | — | `deals` → `Invoice` | 1 | create |
| Create New Product | — | `products` → `Items` | 1 | create |
| Create New Quote | — | `deals` → `Quote` | 1 | create |
| Create New SalesOrder | — | `deals` → `Order` | 1 | create |
| Create Vendor | — | `companies` → `Vendor` | 1 | create |

Across the 10 default templates: 8 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemNumber`, `Line.Ordered`,
  `Line.Description`, `Line.BasePrice`, `Line.AccountID`. 3 templates name the collection
...
system’s response after the write (parent §11). Of this pair’s 10 default templates, 7 carry a
...
- **Response fields it reads them from:** `NewCustomerCode`, `NewInvoiceNumber`, `InventoryId`,
  `NewQuoteNumber`, `NewOrderNumber`, `ShippingID`. The map is written **source-path first,
