---
name: dlake-txdownloaderpro-hubspot/erps/ifs-cloud
description: >-
...
# TxDownloaderPro ← HubSpot — IFS Cloud: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/ifs-cloud.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create Customer with Address | HubSpot companies to IFS Cloud Customer | `companies` → `Customer` | 1 | create |
| Create Order | HubSpot deals to IFS Cloud Order | `deals` → `Order` | 1 | create |
| Create Person with contact | HubSpot contacts to IFS Cloud Person | `contacts` → `Person` | 1 | create |
| Update Customer with Address | HubSpot Update companies to IFS Cloud Update Customer | `companies` → `Customer` | 1 | update |
| Update Person with contact | HubSpot Update contacts to IFS Cloud Update Person | `contacts` → `Person` | 1 | update |
...
- **Members present in the JSON query object:** `selectedFields` (5), `ModuleName` (5), `Where`
  (5). Where a `Where` member is present it is empty. 5 carry a non-empty `selectedFields` list.
...
- **Template path roots used:** `contacts`, `companies`, `deals`, `LineItem`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.CatalogNo`, `Line.CatalogDesc`,
  `Line.BuyQtyDue`, `Line.SaleUnitPrice`. 1 template name the collection through `Line.mainXml`;
