---
name: dlake-txdownloaderpro-hubspot/erps/microsoft-business-central
description: >-
  ...
  `ResultStructure` parts the templates fill for the write back to the CRM. It also lists the 12
...
# TxDownloaderPro ← HubSpot — Microsoft Business Central: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/microsoft-business-central.md`, and that skill’s ERP table
...
| TxDownloaderPro_121_11 | — | `companies` → `Customers` | 2 | create / update |
| Create Custom Order-Transaction Object with Lines | HubSpot Deals to Business Central Custom Order object with lines | `deals` → `Order` | 1 | create |
| TxDownloaderPro_121_14 | — | `deals` → `Order` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_121_13`,
`TxDownloaderPro_121_14`, `TxDownloaderPro_121_1`, `TxDownloaderPro_121_8`,
`TxDownloaderPro_121_11`.
...
- **Objects read:** `deals`, `companies`.
- **Members present in the JSON query object:** `selectedFields` (8), `ModuleName` (8), `Where`
  (8). 4 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ExtraField`. 2 templates name the
...
- **`$FUN_` value tokens the documents carry:** `$FUN_TIMESTAMPTODATE`. The names are what the
...
- **Response fields it reads them from:** `number`, `returnValue1`, `no`. The map is written
...
- **How many:** 12 community templates, across 2 field-process versions.
- **Operations:** 8 carry `IsInsert`, 1 carries `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Contact` (1), `Create New Customer` (1),
  `Create new Sales order` (1), `Create New Sales Order` (1), `New Bills To` (1), `Update Sales
  Order` (1).
- **Names not reproduced:** 6 of these templates carry a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_121_2`, `TxDownloaderPro_121_13`.
