---
name: dlake-txdownloaderpro-hubspot/erps/deltek-vantagepoint
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Deltek Vantagepoint is the source: the 3 default templates the catalogue ships
  ...
  templates fill for the write back to the CRM. It also lists the 6 community templates the
...
# TxDownloaderPro ← HubSpot — Deltek Vantagepoint: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/deltek-vantagepoint.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create Customer | HubSpot companies to Deltek Vantagepoint Firm | `companies` → `Firm` | 1 | create |
| Create New Contact | HubSpot contacts to Deltek Vantagepoint Contact | `contacts` → `Contact` | 1 | create |
| Create Project | HubSpot deals to Deltek Vantagepoint Project | `deals` → `Project` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_39_1`,
`TxDownloaderPro_39_2`, `TxDownloaderPro_39_3`.
...
- **Members present in the JSON query object:** `selectedFields` (3), `ModuleName` (3), `Where`
  (3). Where a `Where` member is present it is empty. 3 carry a non-empty `selectedFields` list.
...
- **Template path roots used:** `contacts`, `companies`, `deals`. A path’s first segment has to
...
- **Response fields it reads them from:** `ClientID`, `ContactID`, `WBSNumber`. The map is
...
- **How many:** 6 community templates, across 3 field-process versions.
- **Operations:** 6 carry `IsInsert`, 0 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create new contact` (1), `Create New Contact` (1),
  `Create New Customer` (1), `Create new firm` (1), `Create New Project` (1).
- **Names not reproduced:** 1 of these templates carries a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_39_1`, `TxDownloaderPro_39_2`,
  `TxDownloaderPro_39_3`.
