---
name: dlake-txdownloaderpro-hubspot/erps/jobboss
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and JobBOSS is the source: the 4 default templates the catalogue ships for this
...
# TxDownloaderPro ← HubSpot — JobBOSS: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/jobboss.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create New Contact | HubSpot contacts to JobBOSS Customer_Contact | `contacts` → `Customer_Contact` | 2 | create / update |
| TxDownloaderPro_27_2 | HubSpot deals to JobBOSS Quote | `deals` → `Quote` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_27_2`,
...
- **`Line.` section members present:** `Line.PartNumber`, `Line.QuotedQuantity`,
  `Line.QuotePrice`, `Line.Comments`, `Line.mainXml`. 2 templates name the collection through
