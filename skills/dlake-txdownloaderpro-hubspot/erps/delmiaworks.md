---
name: dlake-txdownloaderpro-hubspot/erps/delmiaworks
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and DELMIAworks is the source: the 1 default template the catalogue ships for this
...
# TxDownloaderPro ← HubSpot — DELMIAworks: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/delmiaworks.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create Sales Order | HubSpot deals to DELMIAWorks Sales Order | `deals` → `Sales Order` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_156_2`.
...
- **`Line.` section members present:** `Line.ArInvtId`, `Line.BlanketQty`, `Line.UnitPrice`,
  `Line.Note`, `Line.mainXml`. 1 template name the collection through `Line.mainXml`; the
