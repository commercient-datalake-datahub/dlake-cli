---
name: dlake-txdownloaderpro-hubspot/erps/sap-b1
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and SAP B1 is the source: the 8 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 4 community templates the catalogue
...
# TxDownloaderPro ← HubSpot — SAP B1: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/sap-b1.md`, and that skill’s ERP table is what points at it.
...
|---|---|---|---|---|
| Create / Update Customer | HubSpot companies to SAP Business One Customer | `companies` → `Customer` | 2 | create / update |
| Create / Update SalesOrder | HubSpot deals to SAP Business One SalesOrder | `deals` → `SalesOrder` | 2 | create / update |
| Create Contact | HubSpot contacts to SAP Business One Contact | `contacts` → `Contact` | 2 | create / update |
| Create / Update Product | HubSpot products to SAP Business One Product | `products` → `Product` | 1 | create |
| Create / Update WorkOrder | HubSpot deals to SAP Business One Work Order | `deals` → `Work Order` | 1 | create |
...
`TxDownloader_20_3`, `TxDownloader_20_2`, `TxDownloaderPro_20_6`, `TxDownloaderPro_20_8`.
...
- **Objects read:** `companies`, `products`, `deals`, `contacts`.
...
- **Template path roots used:** `companies`, `contacts`, `deals`, `LineItem`, `products`. A
  path’s first segment has to match the element the engine emits, and the document root itself
  is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemCode`, `Line.Quantity`,
  `Line.UnitPrice`, `Line.ItemQuantity`, `Line.ItemPrice`. 3 templates name the collection
...
- **Response fields it reads them from:** `CardCode`, `ItemCode`, `DocEntry`, `OrderNum`,
...
- **How many:** 4 community templates, across 3 field-process versions.
- **Operations:** 4 carry `IsInsert`, 1 carries `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create / Update Customer` (1), `Create New Account`
  (1), `Create New Contact` (1), `Create New Sales Order` (1).
