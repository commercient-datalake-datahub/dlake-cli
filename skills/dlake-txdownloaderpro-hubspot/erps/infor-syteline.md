...
`dlake-txdownloaderpro-hubspot/erps/infor-syteline.md`, and that skill’s ERP table is what
points at it.
...
`Query` does not have one shape across the product (parent §9). For this pair, 24 carry a JSON
...
pair’s 24 default templates, 24 carry a `DefaultProcessStructure`. A parseable document carries
about 14 members.
...
system’s response after the write (parent §11). Of this pair’s 24 default templates, 15 carry a
...
- **How many:** 13 community templates, across 10 field-process versions.
...
- **Template names the catalogue carries:** `New Account` (2), `New Contact` (2), `Update
