---
name: dlake-txdownloaderpro-hubspot/erps/traverse-11
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Traverse 11 is the source: the 4 default templates the catalogue ships for
...
# TxDownloaderPro ← HubSpot — Traverse 11: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/traverse-11.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Customer Import | HubSpot companies to Traverse Customer | `companies` → `Customer` | 1 | create |
| Sales Invoice Import | HubSpot deals to Traverse SalesInvoice | `deals` → `SalesInvoice` | 1 | create |
| SalesOrder Import | HubSpot deals to Traverse SalesOrder | `deals` → `SalesOrder` | 1 | create |
| Vendor Import | HubSpot companies to Traverse Vendor | `companies` → `Vendor` | 1 | create |
...
`TxDownloader_7_3`, `TxDownloader_7_2`, `TxDownloader_7_4`.
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemID`, `Line.Description`,
  `Line.Qty`, `Line.UnitPrice`. 2 templates name the collection through `Line.mainXml`; the
