---
name: dlake-txdownloaderpro-hubspot/erps/sage-100-contractor
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Sage 100 Contractor is the source: the 8 default templates the catalogue ships
...
# TxDownloaderPro ← HubSpot — Sage 100 Contractor: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/sage-100-contractor.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create/Update Contact version 2019 | Hubspot Contacts to Sage100 Contractor ClientContact | `contacts` → `ClientContact`, `JobContact` | 4 | create / update |
| Create/Update Customer version 2019 | Hubspot companies To Sage100 Contractor Customer | `companies` → `Customer` | 2 | create / update |
| Create New Employee version 2019 | Hubspot Contacts to Sage 100 Contractor Employee | `contacts` → `Employee` | 1 | create |
| Create New PurchaseOrder version 2019 | Hubspot Deals to Sage 100 Contractor PurchaseOrder | `deals` → `PurchaseOrder` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_34_5`,
`TxDownloaderPro_34_4`, `TxDownloaderPro_34_2`, `TxDownloaderPro_34_1`.
...
- **Objects read:** `contacts`, `deals`, `companies`.
- **Members present in the JSON query object:** `selectedFields` (8), `ModuleName` (8), `Where`
  (8). 6 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
  is not necessarily a condition on this destination — read the destination page before treating
  it as one. 8 carry a non-empty `selectedFields` list.
...
and are counted but not described. A parseable document carries about 11 members.

- **Template path roots used:** `contacts`, `companies`, `deals`, `ontacts`, `LineItem`,
  `arcustomercode`. A path’s first segment has to match the element the engine emits, and the
  document root itself is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.PartRef`, `Line.Desc`. 1 template
...
- **Response fields it reads them from:** `ObjectID`, `LineID`, `ClientId`. The map is written
...
- **Template names the catalogue carries:** `Create New Customer` (1), `Create New Job` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_34_1`, `TxDownloaderPro_34_3`.
