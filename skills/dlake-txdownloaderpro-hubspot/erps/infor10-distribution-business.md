---
name: dlake-txdownloaderpro-hubspot/erps/infor10-distribution-business
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Infor10 Distribution Business is the source: the 1 default template the
...
# TxDownloaderPro ← HubSpot — Infor10 Distribution Business: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/infor10-distribution-business.md`, and that skill’s ERP
...
|---|---|---|---|---|
| Create Customer in INFOR10DISTRIBUTIONBUSINESS | HubSpot companies to Infor 10 Distribution Business Customer | `companies` → `Customer` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_33_1`.
...
- **Response fields it reads them from:** `customerNumber`. The map is written **source-path
