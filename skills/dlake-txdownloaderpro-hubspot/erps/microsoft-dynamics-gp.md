...
description: >-
  ...
  destination and Microsoft Dynamics GP is the source: the 6 default templates the catalogue
...
`dlake-txdownloaderpro-hubspot/erps/microsoft-dynamics-gp.md`, and that skill’s ERP table is
what points at it.
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ITEMNMBR`, `Line.ITEMDESC`,
  `Line.QUANTITY`, `Line.UNITPRCE`, `Line.UpdateIfExists`, `Line.NONINVEN`, `Line.UOFM`. 1
...
system’s response after the write (parent §11). Of this pair’s 6 default templates, 1 carries a
