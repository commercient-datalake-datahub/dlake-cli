---
name: dlake-txdownloaderpro-hubspot/erps/acumatica-cloud
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Acumatica Cloud is the source: the 6 default templates the catalogue ships for
...
# TxDownloaderPro ← HubSpot — Acumatica Cloud: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/acumatica-cloud.md`, and that skill’s ERP table is what
points at it.
...
`TxDownloader_8_5`, `TxDownloader_8_3`, `TxDownloader_8_2`, `TxDownloader_8_1`.
...
- **Template path roots used:** `companies`, `deals`, `contacts`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.InventoryID`, `Line.Quantity`,
  `Line.UnitPrice`, `Line.Description`, `Line.Amount`, `Line.LineDescription`. 2 templates name
