---
name: dlake-txdownloaderpro-hubspot/erps/epicor-prophet-21-p21
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Epicor Prophet 21 (P21) is the source: the 10 default templates the catalogue
  ships for this pair, the 7 field-process versions they are identified by, what each template
  ...
  parts the templates fill for the write back to the CRM. It also lists the 9 community
...
# TxDownloaderPro ← HubSpot — Epicor Prophet 21 (P21): what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/epicor-prophet-21-p21.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create Contact | HubSpot contacts to Epicor P21 Contact | `contacts` → `Contact` | 2 | create / update |
| Create Customer | HubSpot companies to Epicor P21 Customer | `companies` → `Customer` | 2 | create / update |
| Sales Order | HubSpot deals to Epicor P21 Sales Order | `deals` → `Sales Order` | 2 | create / update |
| Create Customer Export Files | HubSpot companies to Epicor P21 Customer file export | `companies` → `Customer` | 1 | create / update |
| Create Customer File Export | HubSpot companies to Epicor P21 Customer File Export | `companies` → `Customer` | 1 | create |
| TxDownloaderPro_12_3 | HubSpot deals to Epicor P21 Service Order | `deals` → `Service Order` | 1 | create |
| TxDownloaderPro_12_4 | HubSpot companies to Epicor P21 ShipTo | `companies` → `ShipTo` | 1 | create |

Across the 10 default templates: 7 carry `IsInsert`, 4 carry `IsUpdate`, 0 carry `IsDelete`. A
...
`TxDownloaderPro_12_9`, `TxDownloaderPro_12_2`.
...
- **Members present in the JSON query object:** `selectedFields` (10), `ModuleName` (10),
  `Where` (10). Where a `Where` member is present it is empty. 10 carry a non-empty
  `selectedFields` list.
...
- **Template path roots used:** `companies`, `deals`, `contacts`, `LineItem`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemId`, `Line.UnitPrice`,
  `Line.UnitQuantity`, `Line.PartItemId`, `Line.PartExtendedDesc`,
  `Line.PartUnitQuantitySpecified`, `Line.PartUnitQuantity`, `Line.PartUnitPriceSpecified`,
  `Line.PartUnitPrice`. 3 templates name the collection through `Line.mainXml`; the members
  beside it are resolved against that collection’s own root rather than through the header.
- **`$FUN_` value tokens the documents carry:** `$FUN_UNESCAPEXML`, `$FUN_SPLIT`. The names are
...
system’s response after the write (parent §11). Of this pair’s 10 default templates, 4 carry a
...
- **Response fields it reads them from:** `ContactId`, `CustomerCode`, `NewCustomerCode`. The
...
- **How many:** 9 community templates, across 3 field-process versions.
- **Operations:** 7 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create Account` (1), `Create Contact` (1), `Create
  Customer` (1), `Create New Contact` (1), `Create New Customer` (1), `Create New Sales Order`
  (1), `Create Order` (1), `Update Contact` (1), `Update Customer` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_12_5`, `TxDownloaderPro_12_1`,
