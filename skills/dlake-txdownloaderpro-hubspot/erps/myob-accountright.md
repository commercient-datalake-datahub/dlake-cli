---
name: dlake-txdownloaderpro-hubspot/erps/myob-accountright
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and MYOB AccountRight is the source: the 8 default templates the catalogue ships
  for this pair, the 7 field-process versions they are identified by, what each template family
  ...
  templates fill for the write back to the CRM. It also lists the 2 community templates the
...
# TxDownloaderPro ← HubSpot — MYOB AccountRight: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/myob-accountright.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create Customer | HubSpot companies to MYOB AccountRight Customer | `companies` → `Customer` | 2 | create / update |
| Create Contact | HubSpot contacts to MYOB AccountRight Contact | `contacts` → `Contact` | 1 | create |
| Create Customer Location/Address | HubSpot companies to MYOB AccountRight Location | `companies` → `Location` | 1 | create |
| Create Job | HubSpot deals to MYOB AccountRight Job | `deals` → `Job` | 1 | create |
| Create Product | HubSpot products to MYOB AccountRight Product | `products` → `Product` | 1 | create / update |
| Create SalesOrder | HubSpot deals to MYOB AccountRight SalesOrder | `deals` → `SalesOrder` | 1 | create |
| Create ServiceInvoice | HubSpot deals to MYOB AccountRight SalesInvoice | `deals` → `SalesInvoice` | 1 | create |

Across the 8 default templates: 7 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_36_2`,
`TxDownloaderPro_36_1`, `TxDownloaderPro_36_7`, `TxDownloaderPro_36_5`, `TxDownloaderPro_36_6`,
`TxDownloaderPro_36_3`, `TxDownloaderPro_36_4`.
...
- **Members present in the JSON query object:** `selectedFields` (8), `ModuleName` (8), `Where`
  (8). Where a `Where` member is present it is empty. 8 carry a non-empty `selectedFields` list.
...
- **Template path roots used:** `companies`, `deals`, `LineItem`, `products`, `contacts`. A
  path’s first segment has to match the element the engine emits, and the document root itself
  is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemUID`, `Line.Description`,
  `Line.UnitPrice`, `Line.Type`, `Line.ShipQuantity`, `Line.Total`. 2 templates name the
...
system’s response after the write (parent §11). Of this pair’s 8 default templates, 5 carry a
...
- **Response fields it reads them from:** `UID`. The map is written **source-path first,
...
- **Operations:** 2 carry `IsInsert`, 0 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Customer` (1), `Create New Sales Order`
  (1).
- **Field-process versions they belong to:** `TxDownloaderPro_36_1`, `TxDownloaderPro_36_3`.
