---
name: dlake-txdownloaderpro-hubspot/erps/sage-500
description: >-
...
# TxDownloaderPro ← HubSpot — Sage 500: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/sage-500.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| TxDownloaderPro_112_3_1 | HubSpot contacts to Sage 500 Contact | `contacts` → `Contact` | 2 | create / update |
| TxDownloaderPro_112_1_1 | HubSpot companies to Sage 500 Customer | `companies` → `Customer` | 1 | create |
| TxDownloaderPro_112_2_1 | HubSpot deals to Sage 500 SalesOrder | `deals` → `SalesOrder` | 1 | create |
| TxDownloaderPro_112_4_1 | HubSpot deals to Sage 500 Order | `deals` → `Order` | 1 | create |
...
- **Template path roots used:** `contacts`, `deals`, `companies`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.Description`, `Line.QtyOrd`,
  `Line.UnitPrice`, `Line.ItemKey`, `Line.ItemID`. 2 templates name the collection through
