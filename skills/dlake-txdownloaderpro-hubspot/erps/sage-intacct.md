---
name: dlake-txdownloaderpro-hubspot/erps/sage-intacct
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Sage Intacct is the source: the 8 default templates the catalogue ships for
...
# TxDownloaderPro ← HubSpot — Sage Intacct: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/sage-intacct.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Contact | HubSpot contacts to SageIntacct Contact | `contacts` → `Contact` | 1 | create |
| Create New Contract | HubSpot deals to SageIntacct Contract | `deals` → `Contract` | 1 | create |
| Create New Customer | HubSpot companies to SageIntacct Customer | `companies` → `Customer` | 1 | create |
| Create New Invoice | HubSpot deals to SageIntacct Invoice | `deals` → `Invoice` | 1 | create |
| Create New Project/Job | HubSpot deals to SageIntacct Project | `deals` → `Project` | 1 | create |
| Create new Sales Order | HubSpot deals to SageIntacct Sales Order | `deals` → `SalesOrder` | 1 | create |
| Update Customer | HubSpot Update companies to SageIntacct Update Customer | `companies` → `Customer` | 1 | update |
| Update Item | HubSpot Update products to SageIntacct Update Item | `products` → `Item` | 1 | update |

Across the 8 default templates: 6 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A
...
`TxDownloader_10_11`, `TxDownloader_10_1`, `TxDownloader_10_5`, `TxDownloader_10_12`,
`TxDownloader_10_7`, `TxDownloader_10_8`, `TxDownloader_10_3`.
...
- **Template path roots used:** `deals`, `companies`, `contacts`, `LineItem`, `products`. A
  path’s first segment has to match the element the engine emits, and the document root itself
  is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.TransactionAmount`, `Line.Memo`,
  `Line.ItemId`, `Line.ItemDesc`, `Line.Quantity`, `Line.Price`. 2 templates name the collection
