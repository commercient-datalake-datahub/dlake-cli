---
name: dlake-txdownloaderpro-hubspot/erps/epicor-10
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Epicor 10 is the source: the 8 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 16 community templates the catalogue
...
# TxDownloaderPro ← HubSpot — Epicor 10: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/epicor-10.md`, and that skill’s ERP table is what points at
it.
...
`TxDownloaderPro_19_11`, `TxDownloader_19_2`, `TxDownloader_19_1`, `TxDownloader_19_5`,
`TxDownloader_19_10`, `TxDownloader_19_7`.
...
- **`Line.` section members present:** `Line.mainXml`, `Line.PartNum`, `Line.LineDesc`,
  `Line.UnitPrice`, `Line.OrderQty`, `Line.SellingExpectedQty`. 2 templates name the collection
...
- **Response fields it reads them from:** `Company`, `CustID`, `CustNum`, `ShipToNum`,
  `QuoteNum`. The map is written **source-path first, CRM-field second** (parent §11); the wrong
...
- **How many:** 16 community templates, across 6 field-process versions.
- **Operations:** 11 carry `IsInsert`, 5 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Contact` (2), `Update New Customer` (2),
  `Create Case` (1), `Create Contact` (1), `Create Customer` (1), `Create New Company` (1),
  `Create New Customer` (1), `Create New Quote` (1), `Create New Sales Order` (1), `Create
  Product` (1), `Create Sales Order` (1), `Update Case` (1), `Update New Contact` (1).
- **Names not reproduced:** 1 of these templates carries a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_19_12`, `TxDownloader_19_2`,
