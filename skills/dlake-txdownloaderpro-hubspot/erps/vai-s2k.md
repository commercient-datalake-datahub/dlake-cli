...
# TxDownloaderPro ← HubSpot — VAI S2K: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/vai-s2k.md`, and that skill’s ERP table is what points at
it.
...
- **`Line.` section members present:** `Line.SDSFSTMP`, `Line.SDCMP`, `Line.SDSEQ`,
  `Line.SDDES1`, `Line.SDQTY`, `Line.SDPRIC`, `Line.mainXml`. 1 template name the collection
