---
name: dlake-txdownloaderpro-hubspot/erps/xero
description: >-
...
# TxDownloaderPro ← HubSpot — Xero: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/xero.md`, and that skill’s ERP table is what points at it.
...
|---|---|---|---|---|
| Contact Import | HubSpot companies to Xero Contacts | `companies` → `Contacts` | 2 | create / update |
| Invoice Import | HubSpot deals to Xero Invoice | `deals` → `Invoice` | 1 | create |
| Product Items Import | HubSpot products to Xero Product | `products` → `Product` | 1 | create |
...
- **Objects read:** `companies`, `deals`, `products`.
...
- **Template path roots used:** `companies`, `LineItem`, `products`, `deals`. A path’s first
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemCode`, `Line.Description`,
  `Line.Quantity`, `Line.UnitAmount`, `Line.TaxType`, `Line.LineAmount`. 1 template name the
