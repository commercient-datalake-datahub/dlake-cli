---
name: dlake-txdownloaderpro-hubspot/erps/plex
description: >-
...
# TxDownloaderPro ← HubSpot — Plex: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/plex.md`, and that skill’s ERP table is what points at it.
...
|---|---|---|---|---|
| Create Contact | HubSpot contacts to Plex Contact | `contacts` → `Contact` | 1 | create |
| Create Customer | HubSpot companies to Plex Customer | `companies` → `Customer` | 1 | create |
| Create Customer address | HubSpot companies to Plex CustomerAddress | `companies` → `CustomerAddress` | 1 | create |
| Create SalesOrder | HubSpot deals to Plex SalesOrder | `deals` → `SalesOrder` | 1 | create |
...
- **Template path roots used:** `companies`, `contacts`, `deals`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.MainXML`, `Line.Note`, `Line.Price`,
