---
name: dlake-txdownloaderpro-hubspot/erps/microsoft-dynamics-ax
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Microsoft Dynamics AX is the source: the 4 default templates the catalogue
...
# TxDownloaderPro ← HubSpot — Microsoft Dynamics AX: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/microsoft-dynamics-ax.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create Contact | HubSpot contacts to Microsoft Dynamics AX Contact | `contacts` → `Contact` | 1 | create |
| Create Quote | HubSpot deals to Microsoft Dynamics AX Quote | `deals` → `Quote` | 1 | create |
| Update Contact | HubSpot Update contacts to Microsoft Dynamics AX Update Contact | `contacts` → `Contact` | 1 | update |
| Update Quote | HubSpot Update deals to Microsoft Dynamics AX Update Quote | `deals` → `Quote` | 1 | update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_123_2`,
`TxDownloaderPro_123_3`, `TxDownloaderPro_123_4`, `TxDownloaderPro_123_5`.
...
- **Template path roots used:** `contacts`, `deals`, `LineItem`. A path’s first segment has to
  match the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.ProductCode`, `Line.Quantity`. 2
...
- **Response fields it reads them from:** `ContactID`, `QuatationID`. The map is written
