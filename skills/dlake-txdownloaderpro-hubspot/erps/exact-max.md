---
name: dlake-txdownloaderpro-hubspot/erps/exact-max
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Exact MAX is the source: the 1 default template the catalogue ships for this
...
# TxDownloaderPro ← HubSpot — Exact MAX: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/exact-max.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create/Update Customer | HubSpot Update companies to Exact MAX Update Customer | `companies` → `Customer` | 1 | update |

Across the single default template: 0 carry `IsInsert`, 1 carries `IsUpdate`, 0 carry
