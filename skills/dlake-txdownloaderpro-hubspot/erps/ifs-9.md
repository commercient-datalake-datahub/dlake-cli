---
name: dlake-txdownloaderpro-hubspot/erps/ifs-9
description: >-
...
# TxDownloaderPro ← HubSpot — IFS 9: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/ifs-9.md`, and that skill’s ERP table is what points at it.
...
|---|---|---|---|---|
| Create New Customer | HubSpot companies to IFS 9 Customer | `companies` → `Customer` | 1 | create |
| Create New Work Order | HubSpot deals to IFS 9 WorkOrder | `deals` → `WorkOrder` | 1 | create |
| Create/Update Customer Address | HubSpot companies to IFS 9 CustomerAddress | `companies` → `CustomerAddress` | 1 | create |
| TxDownloaderPro_147_4 | HubSpot Update deals to IFS 9 Update WorkOrder | `deals` → `WorkOrder` | 1 | update |
...
- **`Line.` section members present:** `Line.mainXml`, `Line.PART_NUMBER`, `Line.QUANTITY`,
  `Line.SALES_UNIT_PRICE`. 1 template name the collection through `Line.mainXml`; the members
