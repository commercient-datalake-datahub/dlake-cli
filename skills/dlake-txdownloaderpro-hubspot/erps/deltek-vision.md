...
description: >-
  ...
  destination and Deltek Vision is the source: the 7 default templates the catalogue ships for
...
`dlake-txdownloaderpro-hubspot/erps/deltek-vision.md`, and that skill’s ERP table is what points
at it.
...
`TxDownloader_29_1`, `TxDownloaderPro_29_5`, `TxDownloaderPro_29_6`, `TxDownloader_29_2`,
`TxDownloader_29_4`.
...
- **Members present in the JSON query object:** `selectedFields` (7), `ModuleName` (7), `Where`
  (7). Where a `Where` member is present it is empty. 7 carry a non-empty `selectedFields` list.
...
- **Template path roots used:** `contacts`, `deals`, `companies`. A path’s first segment has to
...
- **`$FUN_` value tokens the documents carry:** `$FUN_UNESCAPEXML`, `$FUN_DATEFORMAT`,
  `$FUN_ISNULL`, `$FUN_SUBSTR`. The names are what the templates carry; **no semantics are
...
- **Response fields it reads them from:** `OpportunityID`, `ContactID`, `ClientID`. The map is
