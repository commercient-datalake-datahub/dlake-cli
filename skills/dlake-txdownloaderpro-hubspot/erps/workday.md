---
name: dlake-txdownloaderpro-hubspot/erps/workday
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Workday is the source: the 5 default templates the catalogue ships for this
...
# TxDownloaderPro ← HubSpot — Workday: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/workday.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Contact Import | HubSpot contacts to Workday Contact | `contacts` → `Contact` | 2 | create / update |
| Customer Import | HubSpot companies to Workday Customer | `companies` → `Customer` | 2 | create / update |
| Invoice Import | HubSpot deals to Workday Invoice | `deals` → `Invoice` | 1 | create |
...
`TxDownloaderPro_119_1`, `TxDownloaderPro_119_6`.
...
- **Template path roots used:** `contacts`, `companies`, `deals`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.Sales_Item_Reference`,
  `Line.Line_Item_Description`, `Line.Quantity`, `Line.Unit_Cost`. 1 template name the
...
- **Response fields it reads them from:** `Contact_ID`, `Customer_ID`, `Invoice_ID`. The map is
