---
name: dlake-txdownloaderpro-hubspot/erps/epicor-9-and-9-5
description: >-
...
# TxDownloaderPro ← HubSpot — Epicor 9 and 9.5: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/epicor-9-and-9-5.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Customer | HubSpot companies to Epicor 9 Customer | `companies` → `Customer` | 2 | create / update |
| RMA | HubSpot deals to Epicor 9 RMA | `deals` → `RMA` | 2 | create / update |
| SalesOrder | HubSpot deals to Epicor 9 SalesOrder | `deals` → `SalesOrder` | 1 | create |
...
- **`Line.` section members present:** `Line.PartNum`, `Line.LineDesc`, `Line.mainXml`,
  `Line.ReturnQty`, `Line.RMALine`, `Line.UnitPrice`, `Line.OrderQty`, `Line.DocUnitPrice`,
  `Line.OrderComment`. 3 templates name the collection through `Line.mainXml`; the members
