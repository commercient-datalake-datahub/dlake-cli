---
name: dlake-txdownloaderpro-hubspot/erps/epicor-eclipse
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Epicor Eclipse is the source: the 1 default template the catalogue ships for
...
# TxDownloaderPro ← HubSpot — Epicor Eclipse: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/epicor-eclipse.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create Sales Order | HubSpot deals to Epicor Eclipse Sales Order | `deals` → `Sales Order` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_157_2`.
...
- **`Line.` section members present:** `Line.productId`, `Line.productDescription`,
  `Line.quantity`, `Line.unitPrice`, `Line.mainXml`. 1 template name the collection through
