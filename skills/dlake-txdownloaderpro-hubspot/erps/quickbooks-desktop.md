---
name: dlake-txdownloaderpro-hubspot/erps/quickbooks-desktop
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and QuickBooks Desktop is the source: the 9 default templates the catalogue ships
  for this pair, the 6 field-process versions they are identified by, what each template family
  ...
  templates fill for the write back to the CRM. It also lists the 67 community templates the
...
# TxDownloaderPro ← HubSpot — QuickBooks Desktop: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/quickbooks-desktop.md`, and that skill’s ERP table is what
points at it.
...
| CreateNewEstimate | HubSpot Deals to QuickBooks Estimate | `deals` → `Estimate` | 2 | create / update |
...
Across the 9 default templates: 6 carry `IsInsert`, 3 carry `IsUpdate`, 0 carry `IsDelete`. A
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_5_25`,
`TxDownloaderPro_5_24`, `TxDownloaderPro_5_16`, `TxDownloader_5_1`, `TxDownloader_5_4`,
`TxDownloader_5_2`.
...
- **Objects read:** `companies`, `deals`, `Contacts`.
- **Members present in the JSON query object:** `selectedFields` (9), `ModuleName` (9), `Where`
  (9). 7 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
  is not necessarily a condition on this destination — read the destination page before treating
  it as one. 9 carry a non-empty `selectedFields` list.
...
pair’s 9 default templates, 9 carry a `DefaultProcessStructure`; 1 of those do not parse as JSON
and are counted but not described. A parseable document carries about 15 members.

- **Template path roots used:** `deals`, `Contacts`, `companies`, `LineItem`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.Desc`, `Line.Quantity`, `Line.Amount`,
  `Line.mainXml`, `Line.ItemRefListID`, `Line.ItemListID`, `Line.ItemRef`. 3 templates name the
...
- **`$FUN_` value tokens the documents carry:** `$FUN_IF`, `$FUN_UNESCAPEXML`. The names are
...
system’s response after the write (parent §11). Of this pair’s 9 default templates, 4 carry a
...
- **CRM fields `Part1` writes to:** `externalkey`, `externalKey`, `arcustomercode`. These are
...
- **Response fields it reads them from:** `ListID`, `TxnID`. The map is written **source-path
...
- **How many:** 67 community templates, across 9 field-process versions.
- **Operations:** 35 carry `IsInsert`, 6 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Customer Import` (7), `Create New Customer` (6),
  `Create New Contact` (3), `Create New Sales Order` (3), `New Customer` (3), `Sales Order
  Import` (3), `Create New Account` (2), `Create New Customer Job` (2), `Create New Estimate`
  (2), `New Product` (2), `New Sales Order` (2), `Update Customer` (2), `Create contact` (1),
  `Create Customer` (1), `Create New Customer from Contact` (1), `Create New Inventory Item`
  (1), `Create New Inventory Product` (1), `Create New Product` (1), `Create New Quote` (1),
  `Create Sales Invoice` (1), `Create Sales Order` (1), `Create Sales Order and Job` (1),
  `Create SalesOrder` (1), `New Company` (1), and 8 more.
- **Names not reproduced:** 10 of these templates carry a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_5_25`, `TxDownloaderPro_5_28`,
  `TxDownloaderPro_5_24`, `TxDownloaderPro_5_16`, `TxDownloader_5_1`, `TxDownloader_5_8`,
  `TxDownloader_5_4`, `TxDownloader_5_3`, `TxDownloader_5_2`.
