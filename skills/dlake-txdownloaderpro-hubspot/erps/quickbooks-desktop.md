...
description: >-
  ...
  for this pair, the 5 field-process versions they are identified by, what each template family
...
`TxDownloaderPro_5_24`, `TxDownloader_5_1`, `TxDownloader_5_4`, `TxDownloader_5_2`.
...
and are counted but not described. A parseable document carries about 16 members.

- **Template path roots used:** `Contacts`, `deals`, `companies`, `LineItem`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.ItemListID`, `Line.ItemRef`, `Line.Desc`,
  `Line.Quantity`, `Line.Amount`, `Line.mainXml`, `Line.ItemRefListID`. 1 template name the
