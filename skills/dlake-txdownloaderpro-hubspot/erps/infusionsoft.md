---
name: dlake-txdownloaderpro-hubspot/erps/infusionsoft
description: >-
...
# TxDownloaderPro ← HubSpot — Infusionsoft: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/infusionsoft.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| TxDownloaderPro_100_1 | HubSpot deals to Infusionsoft Order | `deals` → `Order` | 1 | create |
...
- **Members present in the JSON query object:** `selectedFields` (1), `ModuleName` (1), `Where`
  (1). Where a `Where` member is present it is empty. 1 carry a non-empty `selectedFields` list.
...
- **Template path roots used:** `deals`, `LineItem`. A path’s first segment has to match the
