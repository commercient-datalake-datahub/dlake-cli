---
name: dlake-txdownloaderpro-hubspot/erps/aptean-made2manage
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Aptean Made2Manage is the source: the 6 default templates the catalogue ships
...
# TxDownloaderPro ← HubSpot — Aptean Made2Manage: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/aptean-made2manage.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Address | HubSpot companies to Aptean Made2Manage Address | `companies` → `Address` | 2 | create / update |
| Create New Contact | HubSpot contacts to Aptean Made2Manage Contact | `contacts` → `Contact` | 2 | create / update |
| Create New SalesOrder | HubSpot deals to Aptean Made2Manage Order | `deals` → `Order` | 2 | create / update |
...
`TxDownloader_17_2`, `TxDownloader_17_3`.
...
- **Template path roots used:** `contacts`, `companies`, `deals`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.OpportunNum`, `Line.InMastPartNo`,
  `Line.InMastDesc`, `Line.operation`, `Line.identity_column`. 2 templates name the collection
