---
name: dlake-txdownloaderpro-hubspot/erps/applied-epic
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Applied Epic is the source: the 2 default templates the catalogue ships for
...
# TxDownloaderPro ← HubSpot — Applied Epic: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/applied-epic.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Client | HubSpot companies to Applied Epic Client | `companies` → `Client` | 1 | create |
| Create New Contact | HubSpot contacts to Applied Epic Contact | `contacts` → `Contact` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_154_1`,
`TxDownloaderPro_154_2`.
...
- **Members present in the JSON query object:** `selectedFields` (2), `ModuleName` (2), `Where`
  (2). Where a `Where` member is present it is empty. 2 carry a non-empty `selectedFields` list.
...
- **Response fields it reads them from:** `ClientLookupCode`, `ContactID`. The map is written
...
- **Template names the catalogue carries:** `Create New Client` (1), `Create New Contact` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_154_1`, `TxDownloaderPro_154_2`.
