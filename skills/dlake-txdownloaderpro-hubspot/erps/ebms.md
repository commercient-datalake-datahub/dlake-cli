---
name: dlake-txdownloaderpro-hubspot/erps/ebms
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and EBMS is the source: the 6 default templates the catalogue ships for this pair,
  ...
  fill for the write back to the CRM. It also lists the 3 community templates the catalogue
...
# TxDownloaderPro ← HubSpot — EBMS: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/ebms.md`, and that skill’s ERP table is what points at it.
...
|---|---|---|---|---|
| TxDownloaderPro_146_1_1 | HubSpot companies to EBMS Customer | `companies` → `Customer` | 2 | create / update |
| TxDownloaderPro_146_2_1 | HubSpot deals to EBMS ARINV | `deals` → `ARINV` | 2 | create / update |
| TxDownloaderPro_146_3_1 | HubSpot contacts to EBMS ARCONTACTS | `contacts` → `ARCONTACTS` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_146_1`,
`TxDownloaderPro_146_2`, `TxDownloaderPro_146_3`.
...
- **Template path roots used:** `companies`, `deals`, `contacts`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.INVEN`, `Line.DESCR`,
  `Line.M_QUAN_VIS`, `Line.M_PRICE`. 2 templates name the collection through `Line.mainXml`; the
...
- **Response fields it reads them from:** `Guid`. The map is written **source-path first,
...
- **How many:** 3 community templates, across 2 field-process versions.
...
- **Template names the catalogue carries:** `Create Customer from Contact` (1), `Create New
  Customer from Companies` (1), `Create New Sales Order` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_146_1`, `TxDownloaderPro_146_2`.
