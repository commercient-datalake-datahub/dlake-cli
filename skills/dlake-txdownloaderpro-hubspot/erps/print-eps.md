---
name: dlake-txdownloaderpro-hubspot/erps/print-eps
description: >-
...
# TxDownloaderPro ← HubSpot — Print EPS: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/print-eps.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create New Contact | HubSpot contacts to PrintEPS Contact | `contacts` → `Contact` | 1 | create |
| Create new Customer | HubSpot companies to PrintEPS Customer | `companies` → `Customer` | 1 | create |
| Create New Order | HubSpot deals to PrintEPS Order | `deals` → `Order` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_158_2`,
`TxDownloader_158_1`, `TxDownloader_158_3`.
...
- **Template path roots used:** `contacts`, `companies`, `deals`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.Description`, `Line.DetailDescription`,
  `Line.QuantityOrdered`, `Line.SellPrice`, `Line.mainXml`. 1 template name the collection
...
- **Response fields it reads them from:** `Account`, `ResponseJobNumber`. The map is written
