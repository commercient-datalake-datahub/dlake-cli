---
name: dlake-txdownloaderpro-hubspot/erps/infor-sxe
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Infor SXe is the source: the 1 default template the catalogue ships for this
...
# TxDownloaderPro ← HubSpot — Infor SXe: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/infor-sxe.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create/Update Contact | HubSpot contacts to Infor SXe Contacts | `contacts` → `Contacts` | 1 | create / update |

Across the single default template: 1 carries `IsInsert`, 1 carries `IsUpdate`, 0 carry
...
- **Response fields it reads them from:** `ContactId`. The map is written **source-path first,
...
- **Template names the catalogue carries:** `Create Contact` (1).
- **Field-process versions they belong to:** `TxDownloader_114_2`.
