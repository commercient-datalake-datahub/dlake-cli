---
name: dlake-txdownloaderpro-hubspot/erps/jd-edwards
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and JD Edwards is the source: the 5 default templates the catalogue ships for this
...
# TxDownloaderPro ← HubSpot — JD Edwards: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/jd-edwards.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create/update New Contact | HubSpot contacts to JD Edwards Contact | `contacts` → `Contact` | 2 | create / update |
| Create/update New SalesOrder | HubSpot deals to JD Edwards SalesOrder | `deals` → `SalesOrder` | 2 | create / update |
| Create New SalesOrder | HubSpot deals to JD Edwards SalesOrder | `deals` → `SalesOrder` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_16_2`,
`TxDownloader_16_4`, `TxDownloader_16_5`.
...
- **Template path roots used:** `deals`, `contacts`, `LineItem`. A path’s first segment has to
...
- **`Line.` section members present:** `Line.mainXml`, `Line.itemNumber`,
  `Line.quantityOrdered`, `Line.unitPrice`, `Line.requestedDate`, `Line.actionLineACD`,
  `Line.item_cpn`, `Line.item_quantity`, `Line.price_unit_RRP`, `Line.price_unit_discounted`. 3
