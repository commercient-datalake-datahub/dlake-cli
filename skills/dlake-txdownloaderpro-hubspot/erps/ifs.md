...
description: >-
  ...
  destination and IFS is the source: the 22 default templates the catalogue ships for this pair,
  the 22 field-process versions they are identified by, what each template family delivers, the
...
`dlake-txdownloaderpro-hubspot/erps/ifs.md`, and that skill’s ERP table is what points at it.
...
| TxDownloaderPro_116_34 | HubSpot companies to IFS Customer with Address and Tax | `companies` → `Customer` | 1 | create |
...
Across the 22 default templates: 16 carry `IsInsert`, 6 carry `IsUpdate`, 0 carry `IsDelete`. A
...
`Query` does not have one shape across the product (parent §9). For this pair, 22 carry a JSON
...
- **Members present in the JSON query object:** `selectedFields` (22), `ModuleName` (22),
  `Where` (22). Where a `Where` member is present it is empty. 22 carry a non-empty
  `selectedFields` list.
...
pair’s 22 default templates, 22 carry a `DefaultProcessStructure`. A parseable document carries
about 17 members.
...
- **`Line.` section members present:** `Line.mainXml`, `Line.CatalogNo`, `Line.CatalogDesc`,
  `Line.BuyQtyDue`, `Line.SaleUnitPrice`, `Line.PART_NO`, `Line.PART_DESCRIPTION`,
  `Line.BUY_QUANTITY_DUE`, `Line.UNIT_PRICE_IN_OUR_CURRENCY`. 4 templates name the collection
...
system’s response after the write (parent §11). Of this pair’s 22 default templates, 15 carry a
...
- **Response fields it reads them from:** `Cust_CustomerId`, `OrderNo`, `PERSON_PersonId`,
  `PersonId`, `CustomerId`, `PART_PartNo`, `ProjectId`, `ContractNo`, `ContractId`. The map is
