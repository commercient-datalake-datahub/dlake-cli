---
name: dlake-txdownloaderpro-hubspot/erps/sap-business-bydesign
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and SAP Business ByDesign is the source: the 6 default templates the catalogue
...
# TxDownloaderPro ← HubSpot — SAP Business ByDesign: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/sap-business-bydesign.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create New Business Partner | HubSpot companies to SAP Business ByDesign Business Partner | `companies` → `Business Partner` | 2 | create / update |
| Create New Contact | HubSpot companies to SAP Business ByDesign Account | `companies` → `Account` | 2 | create / update |
| Create New Sales Order | HubSpot deals to SAP Business ByDesign Sales Order | `deals` → `Sales Order` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_150_1`,
`TxDownloaderPro_150_3`, `TxDownloaderPro_150_2`.
...
- **Members present in the JSON query object:** `selectedFields` (6), `ModuleName` (6), `Where`
  (6). Where a `Where` member is present it is empty. 6 carry a non-empty `selectedFields` list.
...
- **Template path roots used:** `companies`, `deals`, `LineItem`. A path’s first segment has to
  match the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemProduct_ProductID`,
  `Line.Description`, `Line.ItemScheduleLine_Quantity`. 2 templates name the collection through
...
- **CRM fields `Part1` writes to:** `externalkey`. These are the fields on the flagged record
...
- **Response fields it reads them from:** `ObjectID`. The map is written **source-path first,
...
- **Template names the catalogue carries:** `Create New Account` (1), `Create New Contact` (1),
  `Create New Contact 2` (1), `Create New Sales Order` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_150_1`, `TxDownloaderPro_150_3`,
  `TxDownloaderPro_150_2`.
