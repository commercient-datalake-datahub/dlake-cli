---
name: dlake-txdownloaderpro-hubspot/erps/syspro-6
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and SYSPRO 6 is the source: the 19 default templates the catalogue ships for this
  pair, the 18 field-process versions they are identified by, what each template family
  ...
  templates fill for the write back to the CRM. It also lists the 13 community templates the
...
# TxDownloaderPro ← HubSpot — SYSPRO 6: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/syspro-6.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create New Customer | HubSpot companies to Syspro Customer | `companies` → `Customer` | 2 | create / update |
| Create New Product | HubSpot products to Syspro Product | `products` → `Product` | 1 | create |
| Create New Quote | HubSpot deals to Syspro Quote | `deals` → `Quote` | 1 | create |
| Create New Sales Invoice | HubSpot deals to Syspro Sales Invoice | `deals` → `Sales Invoice` | 1 | create |
| Create New Sales Order | HubSpot deals to Syspro SalesOrder | `deals` → `SalesOrder` | 1 | create |
| Create New Supplier | HubSpot companies to Syspro Supplier | `companies` → `Supplier` | 1 | create |
| Create/Update Purchase Order | HubSpot deals to Syspro Purchase Order | `deals` → `Purchase Order` | 1 | create |
| Delete Customer | HubSpot companies to Syspro Delete Customer | `companies` → `Customer` | 1 | delete |
| Delete Product | HubSpot products to Syspro Delete Product | `products` → `Product` | 1 | delete |
| Delete Purchase Order | HubSpot deals to Syspro Delete Purchase Order | `deals` → `Purchase Order` | 1 | delete |
| Delete Quote | HubSpot deals to Syspro Delete Quote | `deals` → `Quote` | 1 | delete |
| Delete Sales Order | HubSpot deals to Syspro Delete Sales Order | `deals` → `Sales Order` | 1 | delete |
| Delete Supplier | HubSpot companies to Syspro Delete Supplier | `companies` → `Supplier` | 1 | delete |
| TxDownloaderPro_6_10 | HubSpot contacts to Syspro Contact | `contacts` → `Contact` | 1 | create |
| TxDownloaderPro_6_7 | HubSpot Update companies to Syspro Update Customer | `companies` → `Customer` | 1 | update |
| TxDownloaderPro_6_9 | HubSpot Update deals to Syspro Update Sales Order | `deals` → `Sales Order` | 1 | update |
| Update Product | HubSpot Update products to Syspro Update Product | `products` → `Product` | 1 | update |
| Update Supplier | HubSpot Update companies to Syspro Update Supplier | `companies` → `Supplier` | 1 | update |

Across the 19 default templates: 8 carry `IsInsert`, 5 carry `IsUpdate`, 6 carry `IsDelete`. A
...
`TxDownloaderPro_6_29`, `TxDownloaderPro_6_3`, `TxDownloaderPro_6_2`, `TxDownloaderPro_6_17`,
`TxDownloaderPro_6_20`, `TxDownloaderPro_6_11`, `TxDownloaderPro_6_16`, `TxDownloaderPro_6_21`,
`TxDownloaderPro_6_30`, `TxDownloaderPro_6_12`, `TxDownloaderPro_6_19`, `TxDownloaderPro_6_15`,
`TxDownloaderPro_6_18`.
...
- **Objects read:** `companies`, `deals`, `contacts`, `products`.
...
- **Template path roots used:** `companies`, `deals`, `contacts`, `LineItem`, `products`. A
...
- **`Line.` section members present:** `Line.StockCode`, `Line.mainXml`, `Line.LineActionType`,
  `Line.StockDescription`, `Line.OrderQty`, `Line.Price`, `Line.StockType`, `Line.LineAction`,
  `Line.Description`, `Line.QL_OfferNumber1_OfferAction`, `Line.QL_OfferNumber1_Quantity`,
  `Line.QL_OfferNumber1_Price`, `Line.LineAlwaysUsePriceEntered`, `Line.LineAllocationAction`. 3
...
system’s response after the write (parent §11). Of this pair’s 19 default templates, 5 carry a
parseable `DefaultResultStructure`, 14 carry none.
...
- **Response fields it reads them from:** `NewContactFullName`, `NewCustomerCode`, `Quote`,
  `NewSalesOrderNumber`, `NewPurchaseOrderNumber`. The map is written **source-path first,
...
- **How many:** 13 community templates, across 6 field-process versions.
- **Operations:** 5 carry `IsInsert`, 4 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create Custom Field` (1), `Create New Customer
  Syspro 8` (1), `Create new Orders` (1), `New Account` (1), `New Contact` (1), `New Sales
  Order` (1), `Update Account` (1), `Update Quotes` (1).
- **Names not reproduced:** 5 of these templates carry a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_6_10`, `TxDownloaderPro_6_7`,
  `TxDownloaderPro_6_5`, `TxDownloaderPro_6_1`, `TxDownloaderPro_6_29`, `TxDownloaderPro_6_2`.
