---
name: dlake-txdownloaderpro-hubspot/erps/epicor-cloud
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Epicor Cloud is the source: the 8 default templates the catalogue ships for
  ...
  templates fill for the write back to the CRM. It also lists the 27 community templates the
...
# TxDownloaderPro ← HubSpot — Epicor Cloud: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/epicor-cloud.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Contact | HubSpot contacts to Epicor Cloud Contact | `contacts` → `Contact` | 2 | create / update |
| Create New Customer | HubSpot companies to Epicor Cloud Customer | `companies` → `Customer` | 2 | create / update |
| Create New Quote | HubSpot deals to Epicor Cloud Quote | `deals` → `Quote` | 2 | create / update |
| Create New ShipTo | HubSpot companies to Epicor Cloud ShipTo | `companies` → `ShipTo` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_136_2`,
`TxDownloaderPro_136_1`, `TxDownloaderPro_136_3`, `TxDownloaderPro_136_5`.
...
- **Template path roots used:** `companies`, `contacts`, `deals`, `LineItem`. A path’s first
  ...
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.PartNum`, `Line.LineDesc`,
  `Line.OrderQty`, `Line.SellingExpectedQty`, `Line.DocExpUnitPrice`, `Line.QuoteComment`,
  `Line.Company`. 2 templates name the collection through `Line.mainXml`; the members beside it
  are resolved against that collection’s own root rather than through the header.
- **`$FUN_` value tokens the documents carry:** `$FUN_SPLIT`, `$FUN_SUBSTR`, `$FUN_UNESCAPEXML`.
  The names are what the templates carry; **no semantics are claimed for them here** — the
  parent’s §12 is explicit that the platform-side resolver is dotted path substitution only, and
  these tokens are evaluated on the customer’s own host.

## 5. Result structure — what goes back to the CRM

`ResultStructure` is the outbound half: up to four parts, each optional, filled from the source
system’s response after the write (parent §11). Of this pair’s 8 default templates, 4 carry a
...
- **Response fields it reads them from:** `Company`, `CustNum`, `ConNum`, `CustID`, `QuoteNum`,
  `ShipToNum`. The map is written **source-path first, CRM-field second** (parent §11); the
...
- **How many:** 27 community templates, across 4 field-process versions.
- **Operations:** 16 carry `IsInsert`, 10 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Contact` (6), `Create New Customer` (6),
  `Create New Quote` (2), `Create New ShipTo` (2), `Update Contact` (2), `Update New Contact`
  (2), `Update New Customer` (2), `Update customer` (1), `Update New Quote` (1), `Update New
  ShipTo` (1), `Update Quote` (1).
- **Names not reproduced:** 1 of these templates carries a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_136_2`, `TxDownloaderPro_136_1`,
  `TxDownloaderPro_136_3`, `TxDownloaderPro_136_5`.
