---
name: dlake-txdownloaderpro-hubspot/erps/microsoft-dynamics-nav
description: >-
  What the shipped default TxDownloaderPro templates set up when HubSpot is the writeback
  destination and Microsoft Dynamics NAV is the source: the 15 default templates the catalogue
...
# TxDownloaderPro ← HubSpot — Microsoft Dynamics NAV: what the shipped default templates set up
...
`dlake-txdownloaderpro-hubspot/erps/microsoft-dynamics-nav.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create Blanket Order | HubSpot deals to MSDynamicNAV Blanket Sales Order | `deals` → `BlanketSalesOrder` | 2 | create / update |
| Create Contact | HubSpot contacts to MSDynamicNAV Contact | `contacts` → `contact` | 2 | create / update |
| Create Customer | HubSpot companies to MSDynamicNAV Customer | `companies` → `Customer` | 2 | create / update |
| Create Customer Address | HubSpot companies to MSDynamicNAV Ship-to Address | `companies` → `ShipToAddress` | 2 | create / update |
| Create Opportunity | HubSpot deals to MSDynamicNAV Opportunity | `deals` → `Opportunity` | 2 | create / update |
| Create Quote | HubSpot deals to MSDynamicNAV Sales Quote | `deals` → `Sales_Quote` | 2 | create / update |
| Create SalesOrder Return | HubSpot deals to MSDynamicNAV Sales Return Order | `deals` → `SalesOrdeReturn` | 2 | create / update |
| Create SalesOrder | HubSpot deals to MSDynamicNAV Sales Order | `deals` → `SalesOrder` | 1 | create |

Across the 15 default templates: 8 carry `IsInsert`, 7 carry `IsUpdate`, 0 carry `IsDelete`. A
...
`TxDownloaderPro_35_1`, `TxDownloaderPro_35_3`, `TxDownloaderPro_35_7`, `TxDownloaderPro_35_5`,
`TxDownloaderPro_35_6`, `TxDownloaderPro_35_2`, `TxDownloaderPro_35_4`.

## 3. What the query retrieves

`Query` does not have one shape across the product (parent §9). For this pair, 15 carry a JSON
...
- **Objects read:** `deals`, `contacts`, `companies`.
- **Members present in the JSON query object:** `selectedFields` (15), `ModuleName` (15),
  `Where` (15). Where a `Where` member is present it is empty. 15 carry a non-empty
  `selectedFields` list.
...
- **Template path roots used:** `deals`, `companies`, `LineItem`, `contacts`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.Description`, `Line.Unit_Price`,
  `Line.Type`, `Line.No`, `Line.Quantity`, `Line.Variant_Code`, `Line.Qty_To_Assemble_To_Order`.
  7 templates name the collection through `Line.mainXml`; the members beside it are resolved
...
system’s response after the write (parent §11). Of this pair’s 15 default templates, 7 carry a
...
- **Response fields it reads them from:** `No`, `Customer_No`, `Code`. The map is written
