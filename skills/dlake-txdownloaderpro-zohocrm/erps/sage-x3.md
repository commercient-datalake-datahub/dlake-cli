...
description: >-
  ...
  writeback destination and Sage X3 is the source. It does carry 4 community templates for the
...
`dlake-txdownloaderpro-zohocrm/erps/sage-x3.md`, and that skill’s ERP table is what points at
it.
...
- **Operations:** 0 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Import Order` (1), `New Customer` (1), `New/Update
  Contact` (1), `Update Customer` (1).
- **Field-process versions they belong to:** `TxDownloader_23_1`.
