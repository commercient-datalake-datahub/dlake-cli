---
name: dlake-txdownloaderpro-zohocrm/erps/epicor-9-and-9-5
description: >-
  What the shipped default TxDownloaderPro templates set up when Zoho CRM is the writeback
  destination and Epicor 9 and 9.5 is the source: the 2 default templates the catalogue ships
...
# TxDownloaderPro ← Zoho CRM — Epicor 9 and 9.5: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/epicor-9-and-9-5.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Customer | Zoho Account To Epicor9 Customer | `Accounts` → `Customer` | 2 | create / update |
...
- **Members present in the JSON query object:** `selectedFields` (2), `ModuleName` (2), `Where`
  (2). 2 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
...
and are counted but not described. A parseable document carries about 18 members.
...
system’s response after the write (parent §11). Of this pair’s 2 default templates, 1 carries a
