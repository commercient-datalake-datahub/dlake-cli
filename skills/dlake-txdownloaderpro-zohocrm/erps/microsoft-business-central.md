---
name: dlake-txdownloaderpro-zohocrm/erps/microsoft-business-central
description: >-
  What the shipped default TxDownloaderPro templates set up when Zoho CRM is the writeback
  destination and Microsoft Business Central is the source: the 7 default templates the
  catalogue ships for this pair, the 4 field-process versions they are identified by, what each
...
# TxDownloaderPro ← Zoho CRM — Microsoft Business Central: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/microsoft-business-central.md`, and that skill’s ERP table
...
|---|---|---|---|---|
| Create Customer | Zoho Account to Business Central Account | `Account` → `Customer` | 2 | create / update |
| Create New Vendor | Zoho Account to Business Central Vendor | `Account` → `Vendor` | 2 | create / update |
| TxDownloaderPro_121_11 | — | `Accounts` → `Customers` | 2 | create / update |
| TxDownloaderPro_121_10 | Delete Zoho Account to Delete MS Business Central Customer | `Account` → `Customer` | 1 | delete |

Across the 7 default templates: 3 carry `IsInsert`, 3 carry `IsUpdate`, 1 carries `IsDelete`. A
...
`TxDownloaderPro_121_8`, `TxDownloaderPro_121_11`, `TxDownloaderPro_121_10`.
...
- **Members present in the JSON query object:** `selectedFields` (7), `ModuleName` (7), `Where`
  (7). 5 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
...
- **CRM fields `Part1` writes to:** `arcustomercode`, `comrcint_arcustomercode`. These are the
...
- **Response fields it reads them from:** `number`, `no`. The map is written **source-path
