---
name: dlake-txdownloaderpro-zohocrm/erps/sage-live
description: >-
  What the shipped default TxDownloaderPro templates set up when Zoho CRM is the writeback
  destination and Sage Live is the source: the 4 default templates the catalogue ships for this
...
# TxDownloaderPro ← Zoho CRM — Sage Live: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/sage-live.md`, and that skill’s ERP table is what points at
it.
...
`TxDownloader_4_5`, `TxDownloader_4_1`.
...
- **Template path roots used:** `Accounts`, `Sales_Orders`, `Product_Details`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.Product`, `Line.Quantity`, `Line.UnitPrice`,
  `Line.DescriptionLine`, `Line.DiscountType`, `Line.mainXml`. 2 templates name the collection
...
- **Response fields it reads them from:** `NewSalesCreditID`, `NewSalesOrderID`,
  `accountnumber`. The map is written **source-path first, CRM-field second** (parent §11); the
