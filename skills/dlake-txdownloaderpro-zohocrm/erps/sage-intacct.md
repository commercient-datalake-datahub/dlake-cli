...
# TxDownloaderPro ← Zoho CRM — Sage Intacct: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/sage-intacct.md`, and that skill’s ERP table is what points
at it.
...
- **How many:** 6 community templates, across 5 field-process versions.
...
- **Template names the catalogue carries:** `Create New Account` (1), `Create New Contact` (1),
  `Create New Contract` (1), `Create New Customer` (1), `Create New Job` (1).
- **Names not reproduced:** 1 of these templates carries a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloader_10_6`, `TxDownloader_10_1`,
  `TxDownloader_10_13`, `TxDownloader_10_12`, `TxDownloader_10_7`.
