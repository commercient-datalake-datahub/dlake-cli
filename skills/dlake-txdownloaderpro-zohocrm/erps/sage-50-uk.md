---
name: dlake-txdownloaderpro-zohocrm/erps/sage-50-uk
description: >-
...
# TxDownloaderPro ← Zoho CRM — Sage 50 UK: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/sage-50-uk.md`, and that skill’s ERP table is what points at
it.
...
`TxDownloader_3_2`, `TxDownloader_3_1`, `TxDownloader_3_9`.
...
- **Objects read:** `Contacts`, `Quotes`, `Accounts`.
- **Members present in the JSON query object:** `selectedFields` (5), `ModuleName` (5), `Where`
  (5). 4 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
...
- **Template path roots used:** `Contacts`, `Accounts`, `Quotes`, `Product_Details`. A path’s
  first segment has to match the element the engine emits, and the document root itself is never
  part of the path.
- **`Line.` section members present:** `Line.DESCRIPTION`, `Line.DISCOUNT_RATE`,
  `Line.QTY_ORDER`, `Line.UNIT_PRICE`, `Line.mainXml`, `Line.TAX_CODE`. 1 template name the
...
- **Response fields it reads them from:** `ACCOUNT_REF`. The map is written **source-path first,
...
- **How many:** 5 community templates, across 2 field-process versions.
...
- **Template names the catalogue carries:** `Create New Account` (1), `Create New SalesOrder`
  (1), `New Account` (1), `New Sales Order` (1), `Update Sales Order` (1).
- **Field-process versions they belong to:** `TxDownloader_3_2`, `TxDownloader_3_1`.
