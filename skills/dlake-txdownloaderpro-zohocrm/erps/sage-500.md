---
name: dlake-txdownloaderpro-zohocrm/erps/sage-500
description: >-
  What the shipped default TxDownloaderPro templates set up when Zoho CRM is the writeback
  destination and Sage 500 is the source: the 4 default templates the catalogue ships for this
...
# TxDownloaderPro ← Zoho CRM — Sage 500: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/sage-500.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| TxDownloaderPro_112_3_1 | Zoho Contact To Sage500 Contact | `Contacts` → `Contact` | 2 | create / update |
| TxDownloaderPro_112_1_1 | Zoho Account to Sage 500 Customer | `Accounts` → `Customer` | 1 | create |
| TxDownloaderPro_112_4_1 | Zoho Sales Order to Sage 500 SalesOrder | `Sales_Orders` → `Order` | 1 | create |
...
`TxDownloaderPro_112_3`, `TxDownloaderPro_112_4`.
...
- **Template path roots used:** `contacts`, `Sales_Orders`, `Accounts`, `Product_Details`. A
  path’s first segment has to match the element the engine emits, and the document root itself
  is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemID`, `Line.Description`,
  `Line.QtyOrd`, `Line.UnitPrice`. 1 template name the collection through `Line.mainXml`; the
...
- **Response fields it reads them from:** `AddrKey`. The map is written **source-path first,
