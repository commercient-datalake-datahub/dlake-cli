---
name: dlake-txdownloaderpro-zohocrm/erps/sage-200-uk
description: >-
  ...
  templates fill for the write back to the CRM. It also lists the 3 community templates the
...
# TxDownloaderPro ← Zoho CRM — Sage 200 UK: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/sage-200-uk.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create/Update Customer | Update Sage200UK Customer from Zoho Account | `Account`, `Accounts` → `Customer` | 2 | create / update |
| Create Contact | Zoho Contact to Sage 200 UK Customer Contact | `Contacts` → `Contact` | 1 | create |
| TxDownloaderPro_28_2 | Zoho SalesOrder to Sage 200 UK Sales Order | `Sales_Orders` → `Sales Order` | 1 | create |
...
- **Objects read:** `Sales_Orders`, `Contacts`, `Accounts`.
- **Members present in the JSON query object:** `selectedFields` (4), `ModuleName` (4), `Where`
  (4). 2 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
...
- **Template path roots used:** `Accounts`, `Sales_Orders`, `Contacts`, `Product_Details`. A
...
- **Operations:** 2 carry `IsInsert`, 1 carries `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create Contact` (1), `Create Customer` (1), `Update
  Customer` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_28_3`, `TxDownloaderPro_28_1`.
