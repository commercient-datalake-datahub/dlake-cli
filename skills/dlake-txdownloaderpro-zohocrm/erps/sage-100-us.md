...
description: >-
  ...
  destination and Sage 100 (US) is the source: the 10 default templates the catalogue ships for
...
Across the 10 default templates: 5 carry `IsInsert`, 3 carry `IsUpdate`, 3 carry `IsDelete`. A
...
`TxDownloader_2_1`, `TxDownloader_2_7`, `TxDownloader_2_2`, `TxDownloaderPro_2_13`,
`TxDownloader_2_5`, `TxDownloader_2_9`, `TxDownloader_2_15`.
...
- **Objects read:** `Contacts`, `Accounts`, `Products`, `Sales_Orders`.
- **Members present in the JSON query object:** `selectedFields` (10), `ModuleName` (10),
  `Where` (10). 10 of them carry a non-empty `Where`. Its content is not reproduced, and a
  stored `Where` is not necessarily a condition on this destination — read the destination page
  before treating it as one. 10 carry a non-empty `selectedFields` list.
...
pair’s 10 default templates, 10 carry a `DefaultProcessStructure`; 1 of those do not parse as
JSON and are counted but not described. A parseable document carries about 7 members.

- **Template path roots used:** `Accounts`, `Sales_Orders`, `Contacts`, `Products`,
  `Product_Details`, `Account`. A path’s first segment has to match the element the engine
...
system’s response after the write (parent §11). Of this pair’s 10 default templates, 2 carry a
...
- **CRM fields `Part1` writes to:** `Commercient_ArCustomer_Code`, `Commercient_ExternalKey`.
...
- **Response fields it reads them from:** `ARDivisionNo`, `CustomerNo`, `ItemCode`. The map is
