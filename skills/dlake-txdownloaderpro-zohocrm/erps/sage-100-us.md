---
name: dlake-txdownloaderpro-zohocrm/erps/sage-100-us
description: >-
  What the shipped default TxDownloaderPro templates set up when Zoho CRM is the writeback
  destination and Sage 100 (US) is the source: the 17 default templates the catalogue ships for
  ...
  templates fill for the write back to the CRM. It also lists the 24 community templates the
  ...
  generally, and `dlake-txdownloaderpro-zohocrm`, the destination skill this page is a child of,
...
# TxDownloaderPro ← Zoho CRM — Sage 100 (US): what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm` is the Zoho CRM page and carries the destination-side detail: it
...
`dlake-txdownloaderpro-zohocrm/erps/sage-100-us.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Customer | Zoho Account To Sage100 Customer | `Account`, `Accounts` → `Customer` | 2 | create / update |
| Create Purchase Order | Zoho Purchase Order to Sage 100 Purchase Order | `Purchase_Orders` → `Purchase Order` | 2 | create / update |
| Create Vendor | Zoho Account to Sage 100 Vendor | `Account` → `Vendor` | 2 | create / update |
| Create and Update Contact | Zoho Contact TO Create and Update Sage100 Contacts | `Contacts` → `Contacts` | 1 | create / update |
| Create new Product | Zoho Products To Sage100 Products | `Products` → `Item` | 1 | create |
| Create New Sales Order | Zoho Order To Sage100 SalesOrder | `Sales_Orders` → `SalesOrder` | 1 | create |
| Create RMA | Zoho Sales Order to Sage 100 RMA | `Sales_Orders` → `RMA` | 1 | create |
| Delete Contact | Zoho Account to Sage 100 Contact - Delete Contact | `Account` → `Contact` | 1 | delete |
| Delete Customer | Delete Zoho Account to Delete Sage100 Customer | `Account` → `Customer` | 1 | delete |
| Delete Product | Zoho Product to Sage 100 Item - Delete Item | `Products` → `Item` | 1 | delete |
| Delete Purchase Order | Zoho Purchase Order to Sage 100 Purchase Order - Delete Purchase Order | `Purchase_Orders` → `Purchase Order` | 1 | delete |
| Delete Ship To Address | Zoho Account to Sage 100 Ship To Address - Delete Ship To Address | `Accounts` → `ShipToAddress` | 1 | delete |
| Delete Vendor | Zoho Account to Sage 100 Vendor - Delete Vendor | `Account` → `Vendor` | 1 | delete |
| Update Ship To Address | Zoho Account to Sage 100 Ship To Address - Update Ship To Address | `Accounts` → `ShipToAddress` | 1 | update |

Across the 17 default templates: 7 carry `IsInsert`, 5 carry `IsUpdate`, 6 carry `IsDelete`. A
...
`TxDownloader_2_1`, `TxDownloader_2_7`, `TxDownloader_2_2`, `TxDownloader_2_16`,
`TxDownloader_2_8`, `TxDownloaderPro_2_13`, `TxDownloader_2_5`, `TxDownloader_2_9`,
`TxDownloader_2_12`, `TxDownloader_2_18`, `TxDownloader_2_21`, `TxDownloader_2_15`,
`TxDownloader_2_20`.
...
- **Objects read:** `Contacts`, `Accounts`, `Products`, `Sales_Orders`, `Purchase_Orders`.
- **Members present in the JSON query object:** `selectedFields` (17), `ModuleName` (17),
  `Where` (17). 10 of them carry a non-empty `Where`. Its content is not reproduced, and a
...
pair’s 17 default templates, 17 carry a `DefaultProcessStructure`; 1 of those do not parse as
JSON and are counted but not described. A parseable document carries about 8 members.

- **Template path roots used:** `Accounts`, `Purchase_Orders`, `Sales_Orders`,
  `Product_Details`, `Products`, `Contacts`, `Account`. A path’s first segment has to match the
  element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.ItemCode`, `Line.mainXml`, `Line.ItemCodeDesc`,
  `Line.QuantityOrdered`, `Line.UnitCost`, `Line.QuantityReturned`, `Line.InvoiceUnitPrice`,
  `Line.CommentText`. 3 templates name the collection through `Line.mainXml`; the members beside
  it are resolved against that collection’s own root rather than through the header.
- **`$FUN_` value tokens the documents carry:** `$FUN_SUBSTR`, `$FUN_ISNULL`. The names are what
  the templates carry; **no semantics are claimed for them here** — the parent’s §12 is explicit
...
- **Response fields it reads them from:** `ARDivisionNo`, `CustomerNo`, `ItemCode`,
  `PurchaseOrderNo`, `RMANo`. The map is written **source-path first, CRM-field second** (parent
...
- **How many:** 24 community templates, across 6 field-process versions.
- **Operations:** 13 carry `IsInsert`, 6 carry `IsUpdate`, 2 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Contact` (2), `Create New Customer` (2),
  `Update Customer` (2), `Create Account` (1), `Create Contact` (1), `Create Customer` (1),
  `Create New Product` (1), `Create New Quote` (1), `Create New Sales Order` (1), `Create New
  SalesOrder` (1), `Create Sales Order` (1), `Delete Customer` (1), `Delete Order` (1), `New
  Account` (1), `New Contact` (1), `New Sales Order` (1), `Update Contact` (1), `Update New
  Customer` (1), `Update Sales Order` (1).
...
- **Field-process versions they belong to:** `TxDownloader_2_4`, `TxDownloader_2_1`,
  `TxDownloader_2_7`, `TxDownloader_2_2`, `TxDownloader_2_9`, `TxDownloader_2_10`.
...
- `dlake-txdownloaderpro-zohocrm` — the Zoho CRM destination page, which this page is a child
  of: the query shape, marker conventions and mapping conventions this destination uses across
  every ERP, and the ERP table that lists this page and its siblings.
