---
name: dlake-txdownloaderpro-zohocrm/erps/sage-100-contractor-2018
description: >-
  What the shipped default TxDownloaderPro templates set up when Zoho CRM is the writeback
  destination and Sage 100 Contractor 2018 is the source: the 10 default templates the catalogue
  ...
  parts the templates fill for the write back to the CRM. It also lists the 4 community
...
# TxDownloaderPro ← Zoho CRM — Sage 100 Contractor 2018: what the shipped default templates set up
...
`dlake-txdownloaderpro-zohocrm/erps/sage-100-contractor-2018.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create/Update Contact | Zoho Contacts to Sage100 Contractor ClientContact | `Contacts` → `ClientContact`, `JobContact`, `Contact` | 4 | create / update |
| Create/Update Job | Zoho Quotes to Sage 100 Contractor 2018 Job | `Quotes`, `Sales_Orders` → `Job` | 4 | create / update |
| Create/Update Customer | Zoho Account To Sage100 Contractor Customer | `Account` → `Customer` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_115_2`,
`TxDownloaderPro_115_1`, `TxDownloaderPro_115_3`.
...
JSON and are counted but not described. A parseable document carries about 9 members.

- **Template path roots used:** `Contacts`, `Accounts`, `Quotes`, `Sales_Orders`. A path’s first
...
system’s response after the write (parent §11). Of this pair’s 10 default templates, 5 carry a
...
- **Response fields it reads them from:** `ClientId`, `LineID`, `ObjectID`. The map is written
...
- **How many:** 4 community templates, across 2 field-process versions.
- **Operations:** 2 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Contact` (1), `Create New Customer` (1),
  `Update Contact` (1), `Update Customer` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_115_2`, `TxDownloaderPro_115_1`.
