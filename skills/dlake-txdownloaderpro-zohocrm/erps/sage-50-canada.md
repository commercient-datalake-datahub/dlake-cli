...
description: >-
  ...
  destination and Sage 50 Canada is the source: the 7 default templates the catalogue ships for
  ...
  templates fill for the write back to the CRM. It also lists the 8 community templates the
...
`dlake-txdownloaderpro-zohocrm/erps/sage-50-canada.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create New Customer | Zoho Account to Sage50CA Customer | `Accounts` → `Customer` | 2 | create / update |
| Create New ShipToAddress | — | `Accounts` → `ShipToAddress` | 2 | create / update |
...
`TxDownloader_18_1`, `TxDownloader_18_6`, `TxDownloader_18_8`, `TxDownloader_18_5`.
...
- **Members present in the JSON query object:** `selectedFields` (7), `ModuleName` (7), `Where`
  (7). 2 of them carry a non-empty `Where`. Its content is not reproduced, and a stored `Where`
...
- **Template path roots used:** `Accounts`, `Contacts`, `Products`. A path’s first segment has
...
- **Response fields it reads them from:** `InventoryId`, `ShippingID`, `NewCustomerCode`. The
...
- **How many:** 8 community templates, across 6 field-process versions.
...
- **Template names the catalogue carries:** `Create New Account` (1), `Create New Contact` (1),
  `Create New Sales Order` (1), `New Customer` (1), `New Product` (1), `New Quote` (1), `New
  ShipToAddress` (1), `New Vendor` (1).
- **Field-process versions they belong to:** `TxDownloader_18_7`, `TxDownloader_18_1`,
  `TxDownloader_18_6`, `TxDownloader_18_4`, `TxDownloader_18_8`, `TxDownloader_18_5`.
