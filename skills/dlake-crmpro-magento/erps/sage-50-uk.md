...
description: >-
  What the shipped CRMPro templates set up for a Sage 50 UK source pushing into Magento: the
  `CRM_Configuration` row each template inserts (object API names `products`, `customers`, the
  ...
  business outcome of each template group — MAGENTO_GetProduct, MAGENTO_CustomerUpdateKey. Use it
  when standing up or reading a Sage 50 UK → Magento template set, when deciding which templates
...
source's own templates set. This page is `dlake-crmpro-magento/erps/sage-50-uk.md`, and that
...
Sage 50 UK → Magento templates themselves set, read from their `Insert_Query`, `CreateViewQuery`
...
| **MAGENTO_CustomerUpdateKey** | ERP `SF_CUSTOMERSFROMMAGENTO`, `SALES_LEDGER`, `SF_COUNTRY_REGION` data becomes `customers` in Magento. New records are created, existing ones updated, and records removed in the ERP are deleted. | `customers` | `SF_CustomersFromMagento`, `SF_ERP_Salesforce_Clone_SALES_LEDGER`, `SF_ERP_Salesforce_Clone_RM00102`, `Matched_Customers`, `Countries`, `vw_MAGENTO_SAGE50UK_GetCountryCode` |
...
| MAGENTO_CustomerUpdateKey | `customers` | `ExternalKey` | 0 | `vw_Customer` | `vw_MAGENTO_SAGE50UK_CustomerUpdate_Pro` | `''` / `''` | `1` |
...
|---|---|---|---|---|---|
| `vw_MAGENTO_SAGE50UK_CustomerUpdate_Pro` | insert-only | `vw_Customer:` | `vw_Get_Customer:` | `ExternalKey` | `SF_CustomersFromMagento`, `SF_ERP_Salesforce_Clone_SALES_LEDGER`, `SF_ERP_Salesforce_Clone_RM00102`, `Matched_Customers`, `Countries` |
...
- `vw_MAGENTO_SAGE50UK_CustomerUpdate_Pro` reads `vw_Get_Customer:`; no template in this set writes `vw_Get_Customer:`
...
|---|---|---|---|
| MAGENTO_CustomerUpdateKey | `customers` | 9 | `email` → `email`, `FirstName` → `FirstName`, `LastName` → `LastName`, `CUSTOMATTR_commercient_id` → `CUSTOMATTR_commercient_id`, `CUSTOMATTR_customer_number` → `CUSTOMATTR_customer_number` |
...
- Destination objects: `customers`, `product`, `products`, `stockitems`
- Object display names: MAGENTO_CustomerUpdateKey, MAGENTO_GetProduct, MAGENTO_Product, MAGENTO_Update_Inventory
...
adds what the shipped Sage 50 UK → Magento templates set up. `dlake-crmpro-magento` is the
