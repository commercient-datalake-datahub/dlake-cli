...
description: >-
  What the shipped CRMPro templates set up for an Epicor 10 source pushing into Magento: the
  ...
  10 → Magento template set, when deciding which templates to import and activate, or when a run
...
source's own templates set. This page is `dlake-crmpro-magento/erps/epicor-10.md`, and that
...
the product is Magento, which is what this page calls it. What follows is only what the shipped
Epicor 10 → Magento templates themselves set, read from their `Insert_Query`, `CreateViewQuery`
...
|---|---|---|---|
| **MAGENTO_Customer** | ERP `COUNTRIES`, `SF_EROKO_ACCOUNT_MATCHING`, `CUSTCNT` data becomes `customers` in Magento. New records are created, existing ones updated, and records removed in the ERP are deleted. | `customers` | `SF_ERP_Salesforce_Clone_CustomerPriceLst`, `SF_ERP_Salesforce_Clone_Customer`, `SF_ERP_Salesforce_Clone_CustCnt`, `max_PriceLst_Seq`, `SF_EROKO_account_matching`, `Countries` |
...
- Destination objects: `customers`, `customerupdate`, `price view`, `product`, `updateprice`, `invoice`, `orders`, `products`, `sourceitem` and 2 custom objects
- Object display names: MAGENTO_Customer, MAGENTO_customerupdate, MAGENTO_Product, MAGENTO_updateprice, MAGENTO Company, MAGENTO_Get Product, MAGENTO_Inventory Source-Item, MAGENTO_invoice, MAGENTO_OrderTracking, MAGENTO_update Inventory, MAGENTO_update Product and 3 further templates
...
adds what the shipped Epicor 10 → Magento templates set up. `dlake-crmpro-magento` is the
