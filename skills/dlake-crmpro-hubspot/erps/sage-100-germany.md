---
name: dlake-crmpro-hubspot/erps/sage-100-germany
description: >-
  What the shipped CRMPro templates set up for a Sage 100 Germany source pushing into HubSpot: the
  ...
  outcome of each template group — create contact. Use it when standing up or reading a Sage 100
  Germany → HubSpot template set, when deciding which templates to import and activate, or when a
...
# CRMPro → HubSpot — Sage 100 Germany: what the shipped templates set up
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/sage-100-germany.md`, and that
skill's ERP table is what points at it. What follows is only what the shipped Sage 100 Germany →
...
|---|---|---|---|
| **create contact** | ERP `SF_ERP_Salesforce_Clone_KHKAdressen`, `SF_ERP_Salesforce_Clone_KHKAnsprechpartner`, `SF_ERP_Salesforce_Clone_KHKVertreter` data becomes `contact` in HubSpot. New records are created and existing ones updated; none are deleted. | `contact` | `SF_ERP_Salesforce_Clone_KHKAdressen`, `SF_ERP_Salesforce_Clone_KHKAnsprechpartner`, `SF_ERP_Salesforce_Clone_KHKVertreter` |
...
|---|---|---|---|---|---|---|
| `vw_HUBSPOT_NEW_CONTACT` | insert-only | `HUBSPOT_NEW_CONTACT::` | `HUBSPOT_NEW_OWNER::`, `HUBSPOT_NEW_CUSTOMER::` | `RecordKey`, `externalkey` | yes | `SF_ERP_Salesforce_Clone_KHKAdressen`, `SF_ERP_Salesforce_Clone_KHKAnsprechpartner`, `SF_ERP_Salesforce_Clone_KHKVertreter` |
...
|---|---|---|---|
| create contact | `contact` | 12 | `Name1` → `company`, `PostStrasse` → `address`, `PostLand` → `country`, `PostPLZ` → `zip`, `PostOrt` → `city` |
...
- Destination objects: `company`, `contact`, `deal`
- Object display names: update company, upsert contact, upsert deal and a further template
...
adds what the shipped Sage 100 Germany → HubSpot templates set up. `dlake-crmpro-hubspot` is the
