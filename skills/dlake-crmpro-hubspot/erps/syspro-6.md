---
name: dlake-crmpro-hubspot/erps/syspro-6
description: >-
  What the CRMPro template catalogue carries for a SYSPRO 6 source pushing into HubSpot: no
  Standard template ships for this pair, and the 9 community templates it does carry are stated as
  ...
  they write are `company`, `CommercientSF__COMPANYMATCHING__c`,
  `CommercientSF__CONTACTMATCHING__c`, `contact`, `deal` and more. Use it when deciding whether a
  shipped template set exists for a SYSPRO 6 → HubSpot before standing one up, and what the
...
# CRMPro → HubSpot — SYSPRO 6: what the template catalogue carries
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/syspro-6.md`, and that skill's
...
- Destination objects: `company`, `CommercientSF__COMPANYMATCHING__c`, `CommercientSF__CONTACTMATCHING__c`, `contact`, `deal`, `line_item`, `product` and a custom object
- Object display names: create customer, create product, create salesorder line, SYNC COMPANYMATCHING, SYNC CONTACTMATCHING, upsert company, upsert contact, upsert salesorder
...
adds what the SYSPRO 6 → HubSpot template catalogue carries. `dlake-crmpro-hubspot` is the
