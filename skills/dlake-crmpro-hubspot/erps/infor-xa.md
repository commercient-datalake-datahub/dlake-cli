---
name: dlake-crmpro-hubspot/erps/infor-xa
description: >-
  What the CRMPro template catalogue carries for an Infor XA source pushing into HubSpot: no
  Standard template ships for this pair, and the 10 community templates it does carry are stated
  ...
  they write are `line_item`, `lineitem`, `company`, `deal`, `invoice` and more. Use it when
  deciding whether a shipped template set exists for an Infor XA → HubSpot before standing one up,
...
# CRMPro → HubSpot — Infor XA: what the template catalogue carries
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/infor-xa.md`, and that skill's
...
- Destination objects: `line_item`, `lineitem`, `company`, `deal`, `invoice`, `MATCHING__c`, `order`, `product`
- Object display names: upsert order line item, SYNC MATCHING, upsert customer, upsert deal, upsert invoice, upsert invoice line item, upsert line item, upsert order, upsert product
...
adds what the Infor XA → HubSpot template catalogue carries. `dlake-crmpro-hubspot` is the
