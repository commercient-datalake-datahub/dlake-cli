...
description: >-
  ...
  HubSpot: the `CRM_Configuration` row each template inserts (object API names `product`,
  ...
  create company, create contact. Use it when standing up or reading a Dynamics Business Central →
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/dynamics-business-central.md`,
and that skill's ERP table is what points at it. What follows is only what the shipped Dynamics
Business Central → HubSpot templates themselves set, read from their `Insert_Query`,
...
| **create contact** | ERP `SF_ERP_Salesforce_Clone_Contacts` data becomes `contact` in HubSpot. New records are created and existing ones updated; none are deleted. | `contact` | `SF_ERP_Salesforce_Clone_Contacts` |
...
| `vw_HUBSPOT_NEW_CONTACT` | insert-only | `HUBSPOT_NEW_CONTACT::` | `HUBSPOT_NEW_CUSTOMER::` | `RecordKey`, `externalkey` | `SF_ERP_Salesforce_Clone_Contacts` |
...
|---|---|---|---|
| create company | `company` | 8 | `No` → `arcustomercode`, `Name` → `name`, `Address` → `address`, `City` → `city`, `County` → `state` |
| create contact | `contact` | 13 | `E_Mail` → `email`, `Company_Name` → `company`, `Name` → `lastname`, `Phone_No` → `phone`, `Mobile_Phone_No` → `mobilephone` |
| create product | `product` | 8 | `No` → `RecordKey`, `No` → `externalkey`, `No` → `hs_sku`, `Description` → `name`, `Search_Description` → `description` |
...
- Destination objects: `deal`, `company`, `line_item`, `product`, `contact`, `notes`, `BILLTOMATCHING__c`, `commercientdynamicsbusinesscentral_DYNAMICSBUSINESSCENTRAL_Customer`, `DATAMATCHING__c`, `VWHUBSPOTCUSTOMER__c`, `VWHUBSPOTNEWCONTACT__c`, `VWHUBSPOTNEWITEM__c` and 5 custom objects
- Object display names: upsert company, upsert contact, upsert product, upsert shipto, upsert invoice deal, create company, create contact, create product, delete order deal, get company, get products, insert company notes US, 29 more and 11 further templates
...
adds what the shipped Dynamics Business Central → HubSpot templates set up. `dlake-crmpro-hubspot`
