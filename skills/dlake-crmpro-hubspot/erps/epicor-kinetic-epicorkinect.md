---
name: dlake-crmpro-hubspot/erps/epicor-kinetic-epicorkinect
description: >-
  What the shipped CRMPro templates set up for an Epicor Kinetic (EPICORKINECT) source pushing
  into HubSpot: the `CRM_Configuration` row each template inserts (object API names `contact`, the
  ...
  business outcome of each template group — create contact. Use it when standing up or reading an
  Epicor Kinetic (EPICORKINECT) → HubSpot template set, when deciding which templates to import
...
# CRMPro → HubSpot — Epicor Kinetic (EPICORKINECT): what the shipped templates set up
...
`dlake-crmpro-hubspot/erps/epicor-kinetic-epicorkinect.md`, and that skill's ERP table is what
points at it. What follows is only what the shipped Epicor Kinetic (EPICORKINECT) → HubSpot
...
The catalogue carries 60 community templates for this pair. A community template is authored in a
...
- Templates: 60
- Default operations: insert on 60, update on 60, delete on 60
- Marked circular-sync: 0
- Licence groups they span: 5
- Destination objects: `deal`, `line_item`, `company`, `contact`, `product`, `ACCOUNTMATCHING__c`, `CONTACTMATCHING__c`, `DEALMATCHING__c`, `invoice`, `PRODUCTMATCHING__c` and 2 custom objects
- Object display names: upsert company, upsert contact, upsert product, SYNC ACCOUNTMATCHING, create contact, SYNC CONTACTMATCHING, SYNC DEALMATCHING, SYNC PRODUCTMATCHING, upsert Invoice Header, upsert Invoice Line, Upsert Quote, create order Line, 16 more and 10 further templates
...
adds what the shipped Epicor Kinetic (EPICORKINECT) → HubSpot templates set up.
