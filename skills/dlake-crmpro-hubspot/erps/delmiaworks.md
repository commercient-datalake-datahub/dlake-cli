...
description: >-
  ...
  they write are `Company`, `commercientdelmiaworks_DelmiaWorks_SHIP_TOMatching`, `CONTACT`. Use
  it when deciding whether a shipped template set exists for a DELMIAworks → HubSpot before
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/delmiaworks.md`, and that
...
- Destination objects: `Company`, `commercientdelmiaworks_DelmiaWorks_SHIP_TOMatching`, `CONTACT`
- Object display names: insert Company, Insert SHIP_TO, Sync SHIP_TO and 2 further templates
...
adds what the DELMIAworks → HubSpot template catalogue carries. `dlake-crmpro-hubspot` is the
