...
description: >-
  What the shipped CRMPro templates set up for a Sage 50 Cloud source pushing into HubSpot: the
  ...
  outcome of each template group — create contact. Use it when standing up or reading a Sage 50
  Cloud → HubSpot template set, when deciding which templates to import and activate, or when a
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/sage-50-cloud.md`, and that
skill's ERP table is what points at it. What follows is only what the shipped Sage 50 Cloud →
...
|---|---|---|---|
| **create contact** | ERP `SF_ERP_Salesforce_Clone_tcustomr` data becomes `contact` in HubSpot. New records are created and existing ones updated; none are deleted. | `contact` | `SF_ERP_Salesforce_Clone_tcustomr` |
...
|---|---|---|---|---|---|---|
| `vw_HUBSPOT_NEW_CONTACT` | insert-only | `HUBSPOT_NEW_CONTACT::` | `HUBSPOT_NEW_CUSTOMER::` | `RecordKey`, `externalkey` | yes | `SF_ERP_Salesforce_Clone_tcustomr` |
...
|---|---|---|---|
| create contact | `contact` | 10 | `sCntcName` → `firstname`, `sEmail` → `email`, `sPhone1` → `phone`, `sStreet1` → `address`, `sStreet2` → `address2` |
...
- Destination objects: `contact`, `deal`, `line_item`, `company`, `product`
- Object display names: create contact, upsert contact, upsert customer, upsert Invoice, upsert Invoice Line, upsert item, upsert order, upsert order lines
...
adds what the shipped Sage 50 Cloud → HubSpot templates set up. `dlake-crmpro-hubspot` is the
