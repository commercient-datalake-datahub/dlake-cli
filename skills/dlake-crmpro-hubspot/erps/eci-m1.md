---
name: dlake-crmpro-hubspot/erps/eci-m1
description: >-
  What the CRMPro template catalogue carries for an ECi M1 source pushing into HubSpot: no
  ...
  they write are `company`, `contact`, `deal`, `line_item`, `MATCHING__c`. Use it when deciding
  whether a shipped template set exists for an ECi M1 → HubSpot before standing one up, and what
  the community set covers. It extends `dlake-crmpro`, which covers operating CRMPro generally,
  and `dlake-crmpro-hubspot`, the destination skill this page is a child of, which carries the
  HubSpot conventions that hold across every ERP.
generated: true
---
...
# CRMPro → HubSpot — ECi M1: what the template catalogue carries
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/eci-m1.md`, and that skill's
...
- Destination objects: `company`, `contact`, `deal`, `line_item`, `MATCHING__c`
- Object display names: upsert company, SYNC MATCHING, upsert contact, upsert order, upsert order detail, upsert shipto company
...
adds what the ECi M1 → HubSpot template catalogue carries. `dlake-crmpro-hubspot` is the
