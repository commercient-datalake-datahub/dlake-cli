...
description: >-
  What the shipped CRMPro templates set up for an Aptean WorkWise source pushing into HubSpot: the
  ...
  outcome of each template group — create contact. Use it when standing up or reading an Aptean
  WorkWise → HubSpot template set, when deciding which templates to import and activate, or when a
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/aptean-workwise.md`, and that
skill's ERP table is what points at it. What follows is only what the shipped Aptean WorkWise →
...
|---|---|---|---|
| **create contact** | ERP `SF_ERP_Salesforce_Clone_CM_CONTACTS_CUSTOMER`, `SF_ERP_Salesforce_Clone_CUSMAS_SOLDTO` data becomes `contact` in HubSpot. New records are created and existing ones updated; none are deleted. | `contact` | `SF_ERP_Salesforce_Clone_CM_CONTACTS_CUSTOMER`, `SF_ERP_Salesforce_Clone_CUSMAS_SOLDTO` |
...
|---|---|---|---|---|---|
| `vw_HUBSPOT_NEW_CONTACT` | insert-only | `HUBSPOT_NEW_CONTACT::` | `HUBSPOT_NEW_CUSTOMER::` | `RecordKey`, `externalkey` | `SF_ERP_Salesforce_Clone_CM_CONTACTS_CUSTOMER`, `SF_ERP_Salesforce_Clone_CUSMAS_SOLDTO` |
...
- `vw_HUBSPOT_NEW_CONTACT` reads `HUBSPOT_NEW_CUSTOMER::`; no template in this set writes `HUBSPOT_NEW_CUSTOMER::`
...
|---|---|---|---|
| create contact | `contact` | 9 | `ID_CUST, ID_CONTACT` → `externalkey`, `NAME_CONTACT` → `firstname`, `NAME_CONTACT` → `lastname`, `PHONE_MOBILE` → `mobilephone`, `PHONE_BUSINESS` → `phone` |
...
- Destination objects: `deal`, `line_item`, `company`, `contact`, `ACCOUNTMATCHING__c`, `CONTACTMATCHING__c`
- Object display names: create contact, create customer, create invoice, create invoicedetail, create order, create orderdetail, create quote, create quotedetail, SYNC ACCOUNTMATCHING, SYNC CONTACTMATCHING, update contact, update customer
...
adds what the shipped Aptean WorkWise → HubSpot templates set up. `dlake-crmpro-hubspot` is the
