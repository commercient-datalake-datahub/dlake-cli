---
name: dlake-crmpro-hubspot/erps/efacs-8
description: >-
  What the CRMPro template catalogue carries for an EFACS 8 source pushing into HubSpot: no
  ...
  they write are `company`, `contact`, `products`, `ticket`. Use it when deciding whether a
  shipped template set exists for an EFACS 8 → HubSpot before standing one up, and what the
...
# CRMPro → HubSpot — EFACS 8: what the template catalogue carries
...
source's own templates set. This page is `dlake-crmpro-hubspot/erps/efacs-8.md`, and that skill's
...
- Destination objects: `company`, `contact`, `products`, `ticket` and 3 custom objects
- Object display names: upsert address, upsert case, upsert contact, upsert customer, upsert customer child, upsert item and 4 further templates
...
adds what the EFACS 8 → HubSpot template catalogue carries. `dlake-crmpro-hubspot` is the
