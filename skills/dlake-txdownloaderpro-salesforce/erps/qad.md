---
name: dlake-txdownloaderpro-salesforce/erps/qad
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and QAD is the source: the 2 default templates the catalogue ships for this pair,
...
# TxDownloaderPro ← Salesforce — QAD: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/qad.md`, and that skill’s ERP table is what points at it.
...
|---|---|---|---|---|
| Create Customer | Salesforce Account to QAD ERP Customer | `Account` → `Customer` | 1 | create |
| Create SalesOrder | Salesforce Order to QAD ERP SalesOrder | `Order` → `SalesOrder` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_41_1`,
`TxDownloaderPro_41_2`.
...
- **Template path roots used:** `Account`, `Order`, `OrderItems`. A path’s first segment has to
  match the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.part`, `Line.qty_ord`,
  `Line.sod_price`. 1 template name the collection through `Line.mainXml`; the members beside it
...
- **CRM fields `Part1` writes to:** `CommercientSF__Commercient_ArCustomerCode__c`,
  ...
  response, per record.
- **Response fields it reads them from:** `tcBusinessRelationCode`, `OrderNo`. The map is
...
- **Template names the catalogue carries:** `Create New Customer` (1), `Create New Sales Orders`
  (1).
- **Field-process versions they belong to:** `TxDownloaderPro_41_1`, `TxDownloaderPro_41_2`.
