...
- **Objects read:** `OrderItems`, `Account`, `Product2`.
...
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  ...
  `Commercient_Message__c`, `CommercientSF__ExternalKey__c`. These are the columns a user’s flag
...
system’s response after the write (parent §11). Of this pair’s 19 default templates, 7 carry a
parseable `DefaultResultStructure`, 11 carry none, and 1 do not parse and are counted but not
described.
...
- **How many:** 15 community templates, across 8 field-process versions.
- **Operations:** 9 carry `IsInsert`, 6 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create Account` (1), `Create Bill` (1), `Create
  Customer` (1), `Create Invoice` (1), `Create New Customer` (1), `Create New Invoice` (1),
  `Create New Sales Receipt` (1), `Create Vendor` (1), `Update Account` (1), `Update Bill` (1),
