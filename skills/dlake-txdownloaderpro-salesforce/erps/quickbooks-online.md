---
name: dlake-txdownloaderpro-salesforce/erps/quickbooks-online
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and QuickBooks Online is the source: the 5 default templates the catalogue ships
...
# TxDownloaderPro ← Salesforce — QuickBooks Online: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/quickbooks-online.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| TxDownloader_22_1 | Salesforce Account to QuickBooks Online Customer | `Account` → `Customer` | 2 | create |
| TxDownloader_22_14 | Salesforce Account to QuickBooks Online Vendor - Update Vendor | `Account` → `Vendor` | 2 | create / update |
| TxDownloader_22_6 | Salesforce Account to QuickBooks Online Customer - Update Customer | `Account` → `Customer` | 1 | update |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_22_1`,
...
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `Commercient_Import__c`, `Commercient_bCompleted__c`, `Commercient_Message__c`. These are the
...
- **Operators present:** `=`, an empty-string test, `AND`, a null test. The parent’s §12 is the
...
document carries about 13 members.
...
parseable `DefaultResultStructure`, 4 carry none, and 1 do not parse and are counted but not
described.
