...
# TxDownloaderPro ← Salesforce — Microsoft Dynamics GP: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/microsoft-dynamics-gp.md`, and that skill’s ERP table is
what points at it.
...
- **`Line.` section members present:** `Line.ITEMDESC`, `Line.ITEMNMBR`, `Line.LOCNCODE`,
  `Line.QUANTITY`, `Line.UNITPRCE`, `Line.UpdateIfExists`, `Line.mainXml`, `Line.NONINVEN`,
  `Line.UOFM`. 3 templates name the collection through `Line.mainXml`; the members beside it are
