---
name: dlake-txdownloaderpro-salesforce/erps/sage-100-contractor-2018
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Sage 100 Contractor 2018 is the source: the 8 default templates the catalogue
...
# TxDownloaderPro ← Salesforce — Sage 100 Contractor 2018: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/sage-100-contractor-2018.md`, and that skill’s ERP table
...
- **Objects read:** `Contact`, `account`, `Opportunity`.
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `CommercientSF8__ExternalKey__c`, `ExternalKey__c`. These are the columns a user’s flag lands
...
- **Template path roots used:** `Contact`, `Account`, `Opportunity`. A path’s first segment has
...
- **CRM fields `Part1` writes to:** `CommercientSF8__ExternalKey__c`,
