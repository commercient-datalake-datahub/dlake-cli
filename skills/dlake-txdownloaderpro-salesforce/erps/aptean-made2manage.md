...
# TxDownloaderPro ← Salesforce — Aptean Made2Manage: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/aptean-made2manage.md`, and that skill’s ERP table is
what points at it.
...
- **How many:** 8 community templates, across 4 field-process versions.
...
|---|---|
| `CommercientSF21__Aptean_M2M_SYADDR__c` → — | 2 |
| `Contact` → — | 2 |
| `Customer` → — | 2 |
| `Order` → — | 2 |
...
- **Template names the catalogue carries:** `Create New Account` (1), `Create New Address` (1),
  `Create New Contact` (1), `Create New Sales Order` (1), `Update Address` (1), `Update Contact`
  (1), `Update Customer` (1), `Update Sales Order` (1).
- **Field-process versions they belong to:** `TxDownloader_17_4`, `TxDownloader_17_2`,
  `TxDownloader_17_1`, `TxDownloader_17_3`.
