---
name: dlake-txdownloaderpro-salesforce/erps/deltek-vantagepoint
description: >-
...
# TxDownloaderPro ← Salesforce — Deltek Vantagepoint: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/deltek-vantagepoint.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create Customer | Salesforce Account to Deltek Vantagepoint Firm | `Account` → `Firm` | 1 | create |
| Create New Contact | Salesforce Contact to Deltek Vantagepoint Contact | `Contact` → `Contact` | 1 | create |
| Create Project | Salesforce Opportunity to Deltek Vantagepoint Project | `Opportunity` → `Project` | 1 | create |
...
- **Objects read:** `Account`, `Contact`, `Opportunity`.
...
- **Template path roots used:** `Contact`, `Opportunity`, `Account`. A path’s first segment has
...
- **`$FUN_` value tokens the documents carry:** `$FUN_UNESCAPEXML`, `$FUN_DATEFORMAT`,
  `$FUN_STRREPLACE`, `$FUN_SUBSTR`. The names are what the templates carry; **no semantics are
...
- **Template names the catalogue carries:** `Create Customer/Firm` (1), `Create New Project`
  (1).
- **Field-process versions they belong to:** `TxDownloaderPro_39_1`, `TxDownloaderPro_39_3`.
