---
name: dlake-txdownloaderpro-salesforce/erps/exact-globe-next
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Exact Globe Next is the source: the 3 default templates the catalogue ships
...
# TxDownloaderPro ← Salesforce — Exact Globe Next: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/exact-globe-next.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| TxDownloaderPro_129_2_1 | Salesforce Contact to Exact Globe Next Contact | `Contact` → `Contact` | 1 | create |
| TxDownloaderPro_129_3_1 | Salesforce Order to Exact Globe Next Sales Order | `Order` → `SalesOrder` | 1 | create |
| TxDownloaderPro_129_4_1 | Salesforce Product to Exact Globe Next Product | `Product2` → `Item` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_129_2`,
`TxDownloaderPro_129_3`, `TxDownloaderPro_129_4`.
...
- **Objects read:** `Contact`, `OrderItems`, `PricebookEntries`.
- **Child collections pulled in the same query:** `OrderItems`, `PricebookEntries`. A header
...
- **Template path roots used:** `Order`, `Contact`, `Product2`, `OrderItems`. A path’s first
...
- **`Line.` section members present:** `Line.mainXml`, `Line.ItemCode`, `Line.Description`,
  `Line.Quantity`, `Line.Price`. 1 template name the collection through `Line.mainXml`; the
...
- **Response fields it reads them from:** `ContactID`, `SalesOrderNumber`, `ID`. The map is
...
- **How many:** 24 community templates, across 4 field-process versions.
- **Operations:** 12 carry `IsInsert`, 7 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Update Customer` (5), `Create New Contact` (4),
  `Create New Customer` (4), `Create New Sales Order` (4), `New Contact` (1), `New Customer`
  (1), `New PriceBook` (1), `New Product` (1), `New Quote` (1), `Update Contact` (1), `Update
  Product` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_129_1`, `TxDownloaderPro_129_2`,
  `TxDownloaderPro_129_3`, `TxDownloaderPro_129_4`.
