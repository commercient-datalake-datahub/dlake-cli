...
description: >-
  ...
  destination and NetSuite is the source: the 20 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 21 community templates the catalogue
...
# TxDownloaderPro ← Salesforce — NetSuite: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/netsuite.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Invoice | Salesforce Quotes to NetSuite Invoice | `Quote`, `Order` → `Invoice` | 4 | create / update |
...
| Create New Opportunity | Salesforce Opportunity to NetSuite Opportunity | `Opportunity` → `Opportunity` | 2 | create / update |
...
| Create New Project | Salesforce Opportunity to NetSuite Project | `Opportunity` → `Job` | 1 | create |
| UpdateProduct | Salesforce Product and Pricebook to NetSuite Product Pricebook | — → — | 1 | update |

Across the 20 default templates: 10 carry `IsInsert`, 10 carry `IsUpdate`, 0 carry `IsDelete`. A
...
`TxDownloader_9_7`, `TxDownloader_9_9`, `TxDownloader_9_2`, `TxDownloader_9_5`.
...
- **Objects read:** `Contact`, `Account`, `QuoteLineItems`, `Order`, `OpportunityLineItems`,
  `Opportunity`, `OrderItems`, `PricebookEntries`.
...
pair’s 20 default templates, 20 carry a `DefaultProcessStructure`; 2 of those do not parse as
...
system’s response after the write (parent §11). Of this pair’s 20 default templates, 11 carry a
parseable `DefaultResultStructure`, 9 carry none.

| Part | Filled by | What it addresses | Members present |
|---|---|---|---|
| `Part1` | 11 templates | the record the run is already working with | a source-path-to-CRM-field map |
| `Part2` | 0 templates (11 explicitly null) | the child/line records under it | — |
...
- **How many:** 21 community templates, across 5 field-process versions.
...
- **Template names the catalogue carries:** `Create New Customer` (3), `Create Customer` (2),
  `Create New Contact` (2), `Create SalesOrder` (2), `Create and Update Sales Order` (1),
  `Create Contact` (1), `Create New Account` (1), `Create New Sales Order` (1), `Create/Update
  Invoice` (1), `New Contact` (1), `New Customer` (1), `New SalesOrder` (1), `Update Customer`
  (1), `Update SalesOrder` (1).
- **Names not reproduced:** 2 of these templates carry a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloader_9_4`, `TxDownloader_9_1`,
  `TxDownloader_9_3`, `TxDownloader_9_2`, `TxDownloader_9_5`.
