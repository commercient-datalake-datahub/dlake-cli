---
name: dlake-txdownloaderpro-salesforce/erps/eci-spruce
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and ECi Spruce is the source: the 2 default templates the catalogue ships for this
...
# TxDownloaderPro ← Salesforce — ECi Spruce: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/eci-spruce.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Account | Salesforce Account to ECI Spruce Account | `Account` → `Account` | 1 | create |
| Create New Orders | Salesforce Order to ECI Spruce Sales Order | `Order` → `Sales Order` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_155_1`,
`TxDownloaderPro_155_2`.
...
- **CRM fields `Part1` writes to:** `CommercientSF__Commercient_ArCustomerCode__c`,
  ...
  response, per record.
- **Response fields it reads them from:** `AccountNumber`, `ImportSourceId`. The map is written
