...
`dlake-txdownloaderpro-salesforce/erps/infor-sxe.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create/Update Contact | Create and Update Contact from Salesforce | `Contacts` → `Contacts` | 1 | create / update |
| Create/Update Customer | Create and Update Customer from Salesforce | `Account` → `Customer` | 1 | create |

Across the 2 default templates: 2 carry `IsInsert`, 1 carries `IsUpdate`, 0 carry `IsDelete`. A
...
The field-process versions this pair’s default templates belong to: `TxDownloader_114_2`,
`TxDownloader_114_1`.
...
- **Operators present:** `!=`, a null test. The parent’s §12 is the authority on the vocabulary;
  the point here is only which of it these templates use.
...
parseable document carries about 4 members.
...
- **Template names the catalogue carries:** `Create and Update Contact` (1), `Create and Update
  Customer` (1).
- **Field-process versions they belong to:** `TxDownloader_114_2`, `TxDownloader_114_1`.
