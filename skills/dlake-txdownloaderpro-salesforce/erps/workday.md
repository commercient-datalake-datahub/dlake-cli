...
description: >-
  ...
  destination and Workday is the source: the 7 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 14 community templates the catalogue
...
`dlake-txdownloaderpro-salesforce/erps/workday.md`, and that skill’s ERP table is what points at
it.
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_119_8`,
`TxDownloaderPro_119_7`, `TxDownloaderPro_119_5`, `TxDownloaderPro_119_1`,
`TxDownloaderPro_119_6`.
...
- **Objects read:** `Contact`, `Bookings__r`, `Account`, `OrderItems`.
- **Child collections pulled in the same query:** `Bookings__r`, `OrderItems`. A header
...
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `ExternalKey__c`, `Commercient_Import__c`, `CommercientSF__ExternalKey__c`. These are the
...
- **Template path roots used:** `Contact`, `Bookings__r`, `Order`, `Deal__c`, `Account`,
  `OrderItems`. A path’s first segment has to match the element the engine emits, and the
  document root itself is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.Quantity`,
  `Line.Revenue_Category_ID`, `Line.Revenue_Category_Value`, `Line.Contract_Line_Type_Value`,
  `Line.Contract_Line_Description`, `Line.Extended_Amount`,
  `Line.Line_Item_Description_Override`, `Line.Line_Invoice_Memo`,
  `Line.Receivable_Contract_Line_Reference_ID`, `Line.Customer_Contract_Line_Start_Date`,
  `Line.Customer_Contract_Line_End_Date`, `Line.Deferred_Revenue`, `Line.Sales_Item_Reference`,
  `Line.Line_Item_Description`, `Line.Unit_Cost`. 3 templates name the collection through
  `Line.mainXml`; the members beside it are resolved against that collection’s own root rather
  than through the header.
- **`$FUN_` value tokens the documents carry:** `$FUN_UNESCAPEXML`, `$FUN_NOW`,
...
system’s response after the write (parent §11). Of this pair’s 7 default templates, 4 carry a
...
| `Part2` | 1 template (3 explicitly null) | the child/line records under it | `ObjectAPIName`, `LoopFieldTagName`, `LoopFieldIDName`, `FieldName` |
...
- **Response fields it reads them from:** `Contact_ID`, `Customer_Contract_ID`, `Customer_ID`,
  `Invoice_ID`. The map is written **source-path first, CRM-field second** (parent §11); the
  wrong way round resolves to the same silent empty string as a mistyped path.
- **Child objects named by `ObjectAPIName`:** `Booking__c`.
- **Fields the child `FieldName` maps target:** `Contract_Line_Reference__c`.
...
- **How many:** 14 community templates, across 5 field-process versions.
- **Operations:** 7 carry `IsInsert`, 4 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create Contact` (2), `Create Customer` (2), `Create
  Customer Contract` (2), `Customer Contract Amendment` (2), `Update Contact` (2), `Update
  Customer` (2), `Create Premium Seat` (1), `Premium seat` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_119_8`, `TxDownloaderPro_119_7`,
  `TxDownloaderPro_119_5`, `TxDownloaderPro_119_1`, `TxDownloaderPro_119_9`.
