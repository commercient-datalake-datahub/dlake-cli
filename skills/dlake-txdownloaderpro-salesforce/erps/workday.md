...
description: >-
  ...
  destination and Workday is the source: the 4 default templates the catalogue ships for this
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_119_7`,
`TxDownloaderPro_119_5`, `TxDownloaderPro_119_1`.
...
- **Objects read:** `Bookings__r`, `Account`.
- **Child collections pulled in the same query:** `Bookings__r`. A header retrieved without its
...
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `ExternalKey__c`, `Commercient_Import__c`. These are the columns a user’s flag lands in and
...
- **Template path roots used:** `Bookings__r`, `Deal__c`, `Account`. A path’s first segment has
  to match the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.Revenue_Category_ID`,
  `Line.Revenue_Category_Value`, `Line.Contract_Line_Type_Value`,
  `Line.Contract_Line_Description`, `Line.Quantity`, `Line.Extended_Amount`,
  ...
  `Line.Customer_Contract_Line_End_Date`, `Line.Deferred_Revenue`. 2 templates name the
...
- **`$FUN_` value tokens the documents carry:** `$FUN_NOW`. The names are what the templates
...
| `Part2` | 1 template (1 explicitly null) | the child/line records under it | `ObjectAPIName`, `LoopFieldTagName`, `LoopFieldIDName`, `FieldName` |
...
- **Response fields it reads them from:** `Customer_Contract_ID`, `Customer_ID`. The map is
