---
name: dlake-txdownloaderpro-salesforce/erps/southware
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and SouthWare is the source: the 2 default templates the catalogue ships for this
...
# TxDownloaderPro ← Salesforce — SouthWare: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/southware.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Customer | Salesforce Account to SouthWare Customer | `Account` → `Customer` | 1 | create |
| Create New SalesOrder | Salesforce Order to SouthWare Sales Order | `Order` → `SalesOrder` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_134_1`,
`TxDownloader_134_2`.
...
- **Objects read:** `Account`, `OrderItems`.
...
- **Template path roots used:** `Order`, `Account`, `OrderItems`. A path’s first segment has to
  match the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.ItemNo`, `Line.lineitemdesc1`,
  `Line.quantityordered`, `Line.unitprice`, `Line.mainXml`. 1 template name the collection
...
- **Template names the catalogue carries:** `Create New Customer` (1), `Create New SalesOrder`
  (1).
- **Field-process versions they belong to:** `TxDownloader_134_1`, `TxDownloader_134_2`.
