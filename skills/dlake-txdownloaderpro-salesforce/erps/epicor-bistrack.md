---
name: dlake-txdownloaderpro-salesforce/erps/epicor-bistrack
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Epicor BisTrack is the source: the 2 default templates the catalogue ships for
...
# TxDownloaderPro ← Salesforce — Epicor BisTrack: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/epicor-bistrack.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| Create Customer | Salesforce Account to Epicor BisTrack Customer Bill To | `Account` → `CustomerBillTo` | 1 | create |
| Import Contact | Salesforce Order to Epicor BisTrack Sales Order | `Order` → `SalesOrder` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_42_1`,
`TxDownloaderPro_42_2`.
...
- **Template path roots used:** `Account`, `OrderItems`, `Order`. A path’s first segment has to
  match the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.arInvtId`, `Line.blanketQty`,
  `Line.unitPrice`, `Line.note`. 1 template name the collection through `Line.mainXml`; the
...
- **CRM fields `Part1` writes to:** `CommercientSF__Commercient_ArCustomerCode__c`,
  ...
  response, per record.
- **Response fields it reads them from:** `returnValue`. The map is written **source-path first,
