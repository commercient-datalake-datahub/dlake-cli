---
name: dlake-txdownloaderpro-salesforce/erps/maconomy
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Maconomy is the source: the 4 default templates the catalogue ships for this
...
# TxDownloaderPro ← Salesforce — Maconomy: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/maconomy.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create Customer | Salesforce Account to Maconomy Customer | `Account` → `Customer` | 2 | create / update |
| Create Job | Salesforce Opportunity to Maconomy Job | `Opportunity` → `Job` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_40_1`,
`TxDownloaderPro_40_2`.
...
- **Objects read:** `Account`, `Opportunity`.
...
- **Template path roots used:** `Account`, `Opportunity`. A path’s first segment has to match
...
- **CRM fields `Part1` writes to:** `CommercientSF__Commercient_ArCustomerCode__c`,
  ...
  response, per record.
- **Response fields it reads them from:** `customernumber`, `jobnumber`. The map is written
