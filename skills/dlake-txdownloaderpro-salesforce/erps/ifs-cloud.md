---
name: dlake-txdownloaderpro-salesforce/erps/ifs-cloud
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and IFS Cloud is the source: the 5 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 48 community templates the catalogue
...
# TxDownloaderPro ← Salesforce — IFS Cloud: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/ifs-cloud.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create Customer with Address | Salesforce Account to IFS Cloud Customer with Address | `Account` → `Customer` | 1 | create |
| Create Order | Salesforce Order to IFS Cloud Customer Order | `Order` → `Order` | 1 | create |
| Create Person with contact | Salesforce Contacts to IFS Cloud Person with Contact | `Contact` → `Person` | 1 | create |
| Update Customer with Address | Salesforce Update Account to IFS Cloud Customer with Address | `Account` → `Customer` | 1 | update |
| Update Person with contact | Salesforce Update Contacts to IFS Cloud Person with Contact | `Contact` → `Person` | 1 | update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_133_1`,
`TxDownloaderPro_133_6`, `TxDownloaderPro_133_2`, `TxDownloaderPro_133_3`,
`TxDownloaderPro_133_4`.
...
- **Objects read:** `Account`, `OrderItems`, `Contact`.
...
- **Template path roots used:** `Contact`, `Account`, `Order`, `OrderItems`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.CatalogNo`, `Line.CatalogDesc`, `Line.BuyQtyDue`,
  `Line.SaleUnitPrice`, `Line.mainXml`. 1 template name the collection through `Line.mainXml`;
...
- **Response fields it reads them from:** `Cust_CustomerId`, `ORDER_OrderNo`, `PERSON_PersonId`.
...
- **How many:** 48 community templates, across 8 field-process versions.
- **Operations:** 12 carry `IsInsert`, 8 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create a New Part` (4), `Create new Customer with
  Address` (4), `Export Orders to IFS` (4), `Inventory Sales` (4), `Update Customer with
  Address` (4), `Update Person with Contact` (4).
- **Names not reproduced:** 24 of these templates carry a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_133_1`, `TxDownloaderPro_133_10`,
  `TxDownloaderPro_133_2`, `TxDownloaderPro_133_5`, `TxDownloaderPro_133_8`,
  `TxDownloaderPro_133_9`, `TxDownloaderPro_133_3`, `TxDownloaderPro_133_4`.
