---
name: dlake-txdownloaderpro-salesforce/erps/acumatica-cloud
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Acumatica Cloud is the source: the 5 default templates the catalogue ships for
  ...
  templates fill for the write back to the CRM. It also lists the 15 community templates the
...
# TxDownloaderPro ← Salesforce — Acumatica Cloud: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/acumatica-cloud.md`, and that skill’s ERP table is what
points at it.
...
`TxDownloader_8_5`, `TxDownloader_8_2`, `TxDownloader_8_1`.
...
- **Objects read:** `Contact`, `Opportunity`, `OpportunityLineItems`, `Account`.
...
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `ExternalKey__c`, `Discovery_Completed__c`, `CommercientSF__OrderNumber__c`,
  `ROI_Analysis_Completed__c`, `CommercientSF__TrackingNumber__c`, `Commercient_Import__c`.
...
- **Template path roots used:** `Contact`, `Opportunity`, `Account`, `OpportunityLineItems`. A
...
- **`Line.` section members present:** `Line.InventoryID`, `Line.Quantity`, `Line.UnitPrice`,
  `Line.UOM`, `Line.WarehouseID`, `Line.mainXml`, `Line.ExtraField`. 1 template name the
...
- **CRM fields `Part1` writes to:** `ExternalKey__c`, `Acumetica_SalesOrder_No__c`,
...
- **Response fields it reads them from:** `ContactID`, `OrderNbr`, `CustomerID`. The map is
...
- **Operations:** 11 carry `IsInsert`, 4 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Customer` (4), `Create New Contact` (2),
  `Create Customer` (1), `Create New Account` (1), `Create Sales Order` (1), `Create Salesorder`
  (1), `Create SalesOrder` (1), `Update Customer` (1), `Update Customers` (1), `Update Sales
  Orders` (1), `Update SalesOrder` (1).
- **Field-process versions they belong to:** `TxDownloader_8_4`, `TxDownloader_8_2`,
  `TxDownloader_8_1`.
