...
|---|---|---|---|---|
| TxDownloader_8_1 | Update Account from Salesforce to AcumaticaCloude | `Account` → `Customer` | 2 | create / update |
| TxDownloader_8_2 | Create Salesorder for Salesforce to AcumaticaClude | `SalesOrder` → `SalesOrder` | 1 | create |
...
- **Objects read:** `Account`, `OpportunityLineItems`.
...
- **Marker and key columns the queries name:** `Commercient_Import__c`,
  `CommercientSF__Commercient_ArCustomerCode__c`, `Discovery_Completed__c`,
  `CommercientSF__OrderNumber__c`, `ROI_Analysis_Completed__c`,
  `CommercientSF__TrackingNumber__c`. These are the columns a user’s flag lands in and the
...
- **Template path roots used:** `Account`, `Opportunity`, `OpportunityLineItems`. A path’s first
...
- **CRM fields `Part1` writes to:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `Acumetica_SalesOrder_No__c`. These are the fields on the flagged record that carry the source
  system’s key or outcome once the write has happened — the names only; what lands in them is
  the response, per record.
- **Response fields it reads them from:** `CustomerID`, `OrderNbr`. The map is written
