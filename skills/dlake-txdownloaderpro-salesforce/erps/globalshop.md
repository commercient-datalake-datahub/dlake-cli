---
name: dlake-txdownloaderpro-salesforce/erps/globalshop
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and GlobalShop is the source: the 5 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 19 community templates the catalogue
...
# TxDownloaderPro ← Salesforce — GlobalShop: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/globalshop.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Contact | Salesforce Contact to GlobalShop Contact | `Contact` → `Contact` | 2 | create / update |
| Create New Customer | Salesforce Account to GlobalShop Customer | `Account` → `Customer` | 2 | create / update |
| Create New Sales Order | Salesforce Quote to GlobalShop Sales Order | `Quote` → `Sales Order` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_31_3`,
`TxDownloader_31_2`, `TxDownloader_31_1`.
...
- **Objects read:** `Contact`, `Account`, `QuoteLineItems`.
...
- **Template path roots used:** `Account`, `Contact`, `Quote`, `QuoteLineItems`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.PartNumber`,
  `Line.DescriptionPart`, `Line.QtyOrdered`, `Line.OverRidePrice`. 1 template name the
...
- **Response fields it reads them from:** `EXTERNAL_CONTACT_ID`, `GSS_CUSTOMER_NUMBER`,
  `SalesforceId`. The map is written **source-path first, CRM-field second** (parent §11); the
...
- **How many:** 19 community templates, across 3 field-process versions.
- **Operations:** 13 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Sales Order` (4), `Import New Contact`
  (3), `Import New Customer` (3), `Create New Prospect` (2), `Create Contact` (1), `Create New
  Account` (1), `Create New Contact` (1), `Create New Customer` (1), `Create New Quote` (1),
  `Create Update Account` (1), `Create Update Contact` (1).
- **Field-process versions they belong to:** `TxDownloader_31_3`, `TxDownloader_31_2`,
  `TxDownloader_31_1`.
