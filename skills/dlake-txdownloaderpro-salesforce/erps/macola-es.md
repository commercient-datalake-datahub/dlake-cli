---
name: dlake-txdownloaderpro-salesforce/erps/macola-es
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Macola ES is the source: the 2 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 1 community template the catalogue
...
# TxDownloaderPro ← Salesforce — Macola ES: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/macola-es.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| Create New Customer | Create Account from Salesforce to MACOLAES | `Account` → `Customer` | 2 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_14_1`.
...
- **Objects read:** `Account`, `Contacts`.
- **Child collections pulled in the same query:** `Contacts`. A header retrieved without its
...
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `Commercient_IsRecordUpdated__c`. These are the columns a user’s flag lands in and the columns
...
parseable document carries about 122 members.

- **Template path roots used:** `Account`, `Contacts`. A path’s first segment has to match the
...
- **Template names the catalogue carries:** `Create Account XML` (1).
- **Field-process versions they belong to:** `TxDownloader_14_1`.
