---
name: dlake-txdownloaderpro-salesforce/erps/sage-live
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Sage Live is the source: the 5 default templates the catalogue ships for this
...
# TxDownloaderPro ← Salesforce — Sage Live: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/sage-live.md`, and that skill’s ERP table is what points
at it.
...
|---|---|---|---|---|
| TxDownloader_4_2 | Salesforce Order to Sage Live Sales Invoice | `Order`, `Quote`, `Opportunity` → `Invoice` | 3 | create |
| TxDownloader_4_1 | Salesforce Account to Sage Customer | `Account` → `Customer` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloader_4_1`,
`TxDownloader_4_2`.
...
- **Template path roots used:** `Account`, `Order`, `Quote`, `Opportunity`, `OrderItems`,
  `QuoteItems`, `OpportunityItems`, `account`, `salesorder`. A path’s first segment has to match
  the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.ProductId`, `Line.Quantity`, `Line.PriceBookLine`,
  `Line.UnitPrice`, `Line.DescriptionLine`, `Line.DiscountType`, `Line.DiscountValue`,
  `Line.mainXml`. 3 templates name the collection through `Line.mainXml`; the members beside it
