...
description: >-
  ...
  destination and Sage Live is the source: the 7 default templates the catalogue ships for this
...
- **Objects read:** `OrderItems`, `QuoteLineItems`, `OpportunityLineItems`, `Account`.
...
- **Template path roots used:** `Order`, `Account`, `Quote`, `Opportunity`, `OrderItems`,
...
- **`Line.` section members present:** `Line.Quantity`, `Line.UnitPrice`,
  `Line.DescriptionLine`, `Line.DiscountType`, `Line.mainXml`, `Line.ProductId`,
  `Line.PriceBookLine`, `Line.DiscountValue`, `Line.Product`. 5 templates name the collection
...
- **Response fields it reads them from:** `NewSalesCreditID`, `NewSalesOrderID`. The map is
