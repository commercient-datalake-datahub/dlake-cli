---
name: dlake-txdownloaderpro-salesforce/erps/ifs-9
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and IFS 9 is the source: the 4 default templates the catalogue ships for this
...
# TxDownloaderPro ← Salesforce — IFS 9: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/ifs-9.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create New Customer | Salesforce Account to IFS9 Customer | `Account` → `Customer` | 1 | create |
| Create New Work Order | Salesforce Work Order to IFS9 Work Order | `WorkOrder` → `WorkOrder` | 1 | create |
| Create/Update Customer Address | Salesforce Account to IFS9 Customer Address | `Account` → `CustomerAddress` | 1 | create |
| TxDownloaderPro_147_4 | — | `WorkOrder` → `WorkOrder` | 1 | update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_147_4`,
`TxDownloaderPro_147_1`, `TxDownloaderPro_147_3`, `TxDownloaderPro_147_2`.
...
- **Objects read:** `WorkOrderLineItems`, `Account`.
- **Child collections pulled in the same query:** `WorkOrderLineItems`. A header retrieved
...
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  ...
  at all.
- **Operators present:** `!=`, `IN`, `AND`, an empty-string test, `=`, `NOT IN`. The parent’s
...
- **Template path roots used:** `Account`, `WorkOrder`, `WorkOrderLineItems`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.mainXml`, `Line.PART_NUMBER`, `Line.QUANTITY`,
  `Line.SALES_UNIT_PRICE`, `Line.DISCOUNT_PCT`. 1 template name the collection through
...
- **`$FUN_` value tokens the documents carry:** `$FUN_UNESCAPEXML`, `$FUN_STRREPLACE`,
  `$FUN_IF`. The names are what the templates carry; **no semantics are claimed for them here**
  — the parent’s §12 is explicit that the platform-side resolver is dotted path substitution
  only, and these tokens are evaluated on the customer’s own host.

## 5. Result structure — what goes back to the CRM

`ResultStructure` is the outbound half: up to four parts, each optional, filled from the source
system’s response after the write (parent §11). Of this pair’s 4 default templates, 4 carry a
...
- **Response fields it reads them from:** `ResponseWoNo`, `ResponseCustomerId`,
  `ResponseAddressId`. The map is written **source-path first, CRM-field second** (parent §11);
...
- **How many:** 4 community templates, across 4 field-process versions.
- **Operations:** 3 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create / Update Customer` (1), `Create / Update
  Customer Address` (1), `Create Work Order` (1).
- **Names not reproduced:** 1 of these templates carries a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_147_4`, `TxDownloaderPro_147_1`,
  `TxDownloaderPro_147_3`, `TxDownloaderPro_147_2`.
