---
name: dlake-txdownloaderpro-salesforce/erps/exact-max
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Exact MAX is the source: the 2 default templates the catalogue ships for this
...
# TxDownloaderPro ← Salesforce — Exact MAX: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/exact-max.md`, and that skill’s ERP table is what points
at it.
...
The field-process versions this pair’s default templates belong to: `TxDownloader_21_1`.
...
- **Marker and key columns the queries name:** `Commercient_Import__c`,
  `CommercientSF__Commercient_ArCustomerCode__c`, `Commercient_Update__c`,
  `Commercient_Message__C`, `CommercientSF6__SalespersonCode__c`. These are the columns a user’s
  flag lands in and the columns the run writes an outcome back to; which ones are in the `WHERE`
  is what decides whether a record is in scope at all.
...
pair’s 2 default templates, 1 carries a `DefaultProcessStructure` and 1 carries none. A
parseable document carries about 37 members.
...
- **Template names the catalogue carries:** `CreateCustomer` (1), `UpdateCustomer` (1).
- **Field-process versions they belong to:** `TxDownloader_21_1`.
