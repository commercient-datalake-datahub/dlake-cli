---
name: dlake-txdownloaderpro-salesforce/erps/infusionsoft
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Infusionsoft is the source: the 1 default template the catalogue ships for
...
# TxDownloaderPro ← Salesforce — Infusionsoft: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/infusionsoft.md`, and that skill’s ERP table is what
points at it.
...
here exactly as the rows spell it — `Infusionsoft` — and not by a display name.
...
|---|---|---|---|---|
| TxDownloaderPro_100_1 | Salesforce Order to Infusionsoft Order | `Order` → `Order` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_100_1`.
...
- **Marker and key columns the queries name:** `ExternalKey__c`. These are the columns a user’s
...
- **Template path roots used:** `Order`, `OrderItems`. A path’s first segment has to match the
  element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.productName`, `Line.title`,
  `Line.ProductDescription`, `Line.price`, `Line.quantity`, `Line.mainXml`. 1 template name the
...
- **Response fields it reads them from:** `invoidId`. The map is written **source-path first,
