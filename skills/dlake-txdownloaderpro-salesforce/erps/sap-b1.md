...
description: >-
  ...
  destination and SAP B1 is the source: the 10 default templates the catalogue ships for this
...
`TxDownloader_20_2`, `TxDownloaderPro_20_8`.
...
- **Template path roots used:** `Account`, `Contact`, `Opportunity`, `Order`, `Quote`,
  `OpportunityLineItems`, `QuoteLineItems`, `OrderItems`. A path’s first segment has to match
  the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.ItemCode`, `Line.Quantity`, `Line.UnitPrice`,
  `Line.mainXml`. 6 templates name the collection through `Line.mainXml`; the members beside it
...
- **CRM fields `Part1` writes to:** `SAP_DocNum__c`, `SAP_DocEnrty__c`, `ExternalKey__c`,
...
- **Response fields it reads them from:** `DocNum`, `DocEntry`, `CardCode`, `InternalCode`. The
