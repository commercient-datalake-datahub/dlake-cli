---
name: dlake-txdownloaderpro-salesforce/erps/sap-b1
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and SAP B1 is the source: the 11 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 15 community templates the catalogue
...
# TxDownloaderPro ← Salesforce — SAP B1: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/sap-b1.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create / Update SalesOrder | Update SAP SalesOrder from Salesfoce opportunity | `Opportunity`, `SalesOrder`, `Quote` → `sales order`, `SalesOrder` | 6 | create / update |
| Create / Update Customer | Update SAP Customer from Salesforce Account | `Account` → `Salesforce`, `Customer` | 2 | create / update |
| Create Contact | Salesforce Contact To SAPb1 Contact | `Contact` → `Contact` | 2 | create / update |
| Create / Update WorkOrder | Salesforce WorkOrder to SAPB1 WorkOrder | `WorkOrder` → `Work Order` | 1 | create |

Across the 11 default templates: 6 carry `IsInsert`, 5 carry `IsUpdate`, 0 carry `IsDelete`. A
...
The field-process versions this pair’s default templates belong to: `TxDownloader_20_1`,
`TxDownloader_20_2`, `TxDownloaderPro_20_6`, `TxDownloaderPro_20_8`.
...
- **Objects read:** `account`, `opportunitylineitems`, `QuoteLineItems`, `OrderItems`,
  `WorkOrderLineItems`, `Contact`.
- **Child collections pulled in the same query:** `opportunitylineitems`, `QuoteLineItems`,
  `OrderItems`, `WorkOrderLineItems`. A header retrieved without its lines is a query that does
  not name the child collection.
- **Marker and key columns the queries name:** `CommercientSF__Commercient_ArCustomerCode__c`,
  `CommercientSF__ExternalKey__c`, `ExternalKey__c`. These are the columns a user’s flag lands
...
- **Template path roots used:** `Account`, `Contact`, `Opportunity`, `Order`, `WorkOrder`,
  `Quote`, `OpportunityLineItems`, `QuoteLineItems`, `OrderItems`, `WorkOrderLineItems`. A
  path’s first segment has to match the element the engine emits, and the document root itself
  is never part of the path.
- **`Line.` section members present:** `Line.ItemCode`, `Line.mainXml`, `Line.Quantity`,
  `Line.UnitPrice`, `Line.ItemQuantity`, `Line.ItemPrice`. 7 templates name the collection
...
system’s response after the write (parent §11). Of this pair’s 11 default templates, 6 carry a
...
- **CRM fields `Part1` writes to:** `ExternalKey__c`, `SAP_DocNum__c`, `SAP_DocEnrty__c`,
  `CommercientSF__Commercient_ArCustomerCode__c`, `Order.DocNum__c`. These are the fields on the
...
- **Response fields it reads them from:** `DocNum`, `DocEntry`, `CardCode`, `OrderNum`,
  `InternalCode`. The map is written **source-path first, CRM-field second** (parent §11); the
...
- **How many:** 15 community templates, across 3 field-process versions.
- **Operations:** 15 carry `IsInsert`, 5 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Contact` (3), `Create New Customer` (3),
  `Create New Account` (2), `Create / Update SalesOrder` (1), `Create and Update Contact` (1),
  `Create and Update Customer` (1), `Create and Update Sales Order` (1), `Create Contact` (1),
  `Create Salesorder` (1), `create Update Customer` (1).
- **Field-process versions they belong to:** `TxDownloader_20_1`, `TxDownloader_20_2`,
  `TxDownloaderPro_20_8`.
