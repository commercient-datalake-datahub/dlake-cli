---
name: dlake-txdownloaderpro-salesforce/erps/deltek-vision
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and Deltek Vision is the source: the 9 default templates the catalogue ships for
  this pair, the 7 field-process versions they are identified by, what each template family
  ...
  templates fill for the write back to the CRM. It also lists the 18 community templates the
...
# TxDownloaderPro ← Salesforce — Deltek Vision: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/deltek-vision.md`, and that skill’s ERP table is what
points at it.
...
The field-process versions this pair’s default templates belong to: `TxDownloader_29_3`,
`TxDownloader_29_1`, `TxDownloaderPro_29_7`, `TxDownloaderPro_29_5`, `TxDownloaderPro_29_6`,
`TxDownloader_29_2`, `TxDownloader_29_4`.
...
- **Objects read:** `Contact`, `Account`, `Task`, `Opportunity`.
...
- **Template path roots used:** `Contact`, `Opportunity`, `Task`, `Account`. A path’s first
...
- **`$FUN_` value tokens the documents carry:** `$FUN_UNESCAPEXML`, `$FUN_DATEFORMAT`,
  `$FUN_STRREPLACE`, `$FUN_ISNULL`, `$FUN_SUBSTR`. The names are what the templates carry; **no
  semantics are claimed for them here** — the parent’s §12 is explicit that the platform-side
  resolver is dotted path substitution only, and these tokens are evaluated on the customer’s
  own host.

## 5. Result structure — what goes back to the CRM

`ResultStructure` is the outbound half: up to four parts, each optional, filled from the source
system’s response after the write (parent §11). Of this pair’s 9 default templates, 5 carry a
...
- **Response fields it reads them from:** `OpportunityID`, `ContactID`, `ClientID`,
  `ActivityID`. The map is written **source-path first, CRM-field second** (parent §11); the
...
- **How many:** 18 community templates, across 6 field-process versions.
- **Operations:** 15 carry `IsInsert`, 15 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create Contact` (1), `Create Customer` (1), `Create
  New and Update Activity Task - Accounts` (1), `Create New and Update Activity Task - Contacts`
  (1), `Create New and Update Activity Task - Opportunities` (1), `Create Opportunity` (1),
  `Update Contact` (1), `Update Customer` (1), `Update Opportunity` (1).
- **Names not reproduced:** 9 of these templates carry a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloader_29_3`, `TxDownloader_29_1`,
  `TxDownloaderPro_29_7`, `TxDownloaderPro_29_5`, `TxDownloader_29_2`, `TxDownloader_29_4`.
