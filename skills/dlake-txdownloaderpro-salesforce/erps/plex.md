---
name: dlake-txdownloaderpro-salesforce/erps/plex
description: >-
  The catalogue ships no default TxDownloaderPro template for the pair where Salesforce is the
  writeback destination and Plex is the source. It does carry 51 community templates for the
  pair, across 4 field-process versions, and this page lists them: how many there are, which
...
# TxDownloaderPro ← Salesforce — Plex: no default templates, and the community set the catalogue carries
...
`dlake-txdownloaderpro-salesforce/erps/plex.md`, and that skill’s ERP table is what points at
it.
...
- **How many:** 51 community templates, across 4 field-process versions.
- **Operations:** 16 carry `IsInsert`, 21 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Contact` (7), `Create New Customer` (7),
  `Update Contact` (7), `Update Customer` (7), `Update Customer Address` (7), `New Customer
  Address` (5), `New Sales Order` (5), `Export Sales Order to Plex` (2), `Create Customer
  Address` (1), `Create New Customer Address` (1).
- **Names not reproduced:** 2 of these templates carry a name that is not a product artefact
  name, and it is not printed here.
- **Field-process versions they belong to:** `TxDownloaderPro_37_3`, `TxDownloaderPro_37_1`,
  `TxDownloaderPro_37_4`, `TxDownloaderPro_37_2`.
