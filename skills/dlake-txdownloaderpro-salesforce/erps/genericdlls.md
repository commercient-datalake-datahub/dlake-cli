...
description: >-
  ...
  writeback destination and GenericDLLs is the source. It does carry 26 community templates for
...
# TxDownloaderPro ← Salesforce — GenericDLLs: no default templates, and the community set the catalogue carries
...
`dlake-txdownloaderpro-salesforce/erps/genericdlls.md`, and that skill’s ERP table is what
points at it.
...
- **How many:** 26 community templates, across 2 field-process versions.
- **Operations:** 13 carry `IsInsert`, 4 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Customer` (2), `Create Contact` (1),
  `Create Invoice` (1), `Create New contact` (1), `Create New Contact` (1), `Create New
  Opportunity` (1), `Create New Quote` (1), `Create New Sales Order` (1), `Create Sales Order`
  (1), `Create Salesorder` (1), `Create/Update Contact` (1), `CustomerImport` (1),
  `CustomerUpdate` (1), `Generate SalesOrder CSV` (1), `New Account` (1), `New Contact` (1),
  `New Customer` (1), `New Sales Order` (1), `New SalesOrder` (1), `New/Update Customer` (1),
  `ProductImport` (1), `SalesOrderImport` (1), `Update Pricebook` (1).
