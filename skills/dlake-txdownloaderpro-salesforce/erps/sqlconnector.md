---
name: dlake-txdownloaderpro-salesforce/erps/sqlconnector
description: >-
  The catalogue ships no default TxDownloaderPro template for the pair where Salesforce is the
  writeback destination and SQLConnector is the source. It does carry 4 community templates for
...
# TxDownloaderPro ← Salesforce — SQLConnector: no default templates, and the community set the catalogue carries
...
`dlake-txdownloaderpro-salesforce/erps/sqlconnector.md`, and that skill’s ERP table is what
points at it.
...
- **How many:** 4 community templates, across 1 field-process version.
- **Operations:** 4 carry `IsInsert`, 2 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create and Update Contact` (2), `Create Contact`
  (2).
