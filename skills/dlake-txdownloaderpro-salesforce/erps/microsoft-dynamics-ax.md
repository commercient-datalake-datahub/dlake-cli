---
name: dlake-txdownloaderpro-salesforce/erps/microsoft-dynamics-ax
description: >-
...
# TxDownloaderPro ← Salesforce — Microsoft Dynamics AX: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/microsoft-dynamics-ax.md`, and that skill’s ERP table is
what points at it.
...
|---|---|---|---|---|
| Create Contact | Salesforce Contact to MSDynamicAx Contact | `Contact` → `Contact` | 1 | create |
| Create Quote | — | `Quote` → `Quote` | 1 | create |
| Update Contact | Salesforce Contact to MSDynamicAx Update Contact | `Contact` → `Contact` | 1 | update |
| Update Quote | — | `Quote` → `Quote` | 1 | update |
...
- **Objects read:** `Contact`, `QuoteLineItems`.
...
- **Template path roots used:** `Contact`, `Quote`, `QuoteLineItems`. A path’s first segment has
...
- **`$FUN_` value tokens the documents carry:** `$FUN_ISNULL`, `$FUN_STRREPLACE`. The names are
