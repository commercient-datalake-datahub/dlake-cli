...
description: >-
  ...
  destination and Aptean Encompix is the source: the 1 default template the catalogue ships for
...
`dlake-txdownloaderpro-salesforce/erps/aptean-encompix.md`, and that skill’s ERP table is what
points at it.
...
|---|---|---|---|---|
| TxDownloaderPro_144_1_1 | Salesforce Quote to Aptean Encompix Quote | `Quote` → `Quote` | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_144_1`.
...
- **Template path roots used:** `Quote`, `QuoteLineItems`. A path’s first segment has to match
  the element the engine emits, and the document root itself is never part of the path.
- **`Line.` section members present:** `Line.ItemNumber`, `Line.ItemDesc`, `Line.QuoteQty`,
  `Line.BasePrice`, `Line.DueDate`, `Line.mainXml`. 1 template name the collection through
