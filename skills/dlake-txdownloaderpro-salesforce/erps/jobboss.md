---
name: dlake-txdownloaderpro-salesforce/erps/jobboss
description: >-
  What the shipped default TxDownloaderPro templates set up when Salesforce is the writeback
  destination and JobBOSS is the source: the 5 default templates the catalogue ships for this
  ...
  fill for the write back to the CRM. It also lists the 7 community templates the catalogue
...
# TxDownloaderPro ← Salesforce — JobBOSS: what the shipped default templates set up
...
`dlake-txdownloaderpro-salesforce/erps/jobboss.md`, and that skill’s ERP table is what points at
it.
...
|---|---|---|---|---|
| Create New Contact | Salesforce Contact to JobBOSS Customer Contact | `Contact` → `Customer_Contact` | 2 | create / update |
| TxDownloaderPro_27_2 | Salesforce Opportunity to JobBOSS Quote | `Opportunity` → `Quote` | 2 | create / update |
| TxDownloaderPro_27_1 | — | — → — | 1 | create |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_27_1`,
`TxDownloaderPro_27_2`, `TxDownloaderPro_27_3`.
...
- **Objects read:** `Account`, `OpportunityLineItems`, `Contact`.
...
- **Marker and key columns the queries name:** `Commercient_Import__c`,
  `CommercientSF__Commercient_ArCustomerCode__c`, `ExternalKey__c`,
  `CommercientSF__ExternalKey__c`. These are the columns a user’s flag lands in and the columns
...
pair’s 5 default templates, 4 carry a `DefaultProcessStructure` and 1 carries none. A parseable
document carries about 9 members.

- **Template path roots used:** `Opportunity`, `Contact`, `OpportunityLineItems`. A path’s first
  segment has to match the element the engine emits, and the document root itself is never part
  of the path.
- **`Line.` section members present:** `Line.PartNumber`, `Line.QuotedQuantity`,
  `Line.QuotePrice`, `Line.Comments`, `Line.mainXml`, `Line.LineItemID`. 2 templates name the
...
- **Response fields it reads them from:** `ID`, `ContactId`. The map is written **source-path
...
- **How many:** 7 community templates, across 3 field-process versions.
- **Operations:** 2 carry `IsInsert`, 3 carry `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Template names the catalogue carries:** `Create New Customer` (1), `Create New Quote` (1),
  `New Account` (1), `New Contact` (1), `Update Customer` (1), `Update Quote` (1), `Updated
  Quote` (1).
- **Field-process versions they belong to:** `TxDownloaderPro_27_1`, `TxDownloaderPro_27_2`,
  `TxDownloaderPro_27_3`.
