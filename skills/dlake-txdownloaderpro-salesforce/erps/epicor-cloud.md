...
description: >-
  ...
  destination and Epicor Cloud is the source: the 9 default templates the catalogue ships for
...
`dlake-txdownloaderpro-salesforce/erps/epicor-cloud.md`, and that skill’s ERP table is what
points at it.
...
| Create New ShipTo | Salesforce Account to Epicor Cloud ShipTo | `Account` → `ShipTo` | 2 | create / update |
...
The field-process versions this pair’s default templates belong to: `TxDownloaderPro_136_4`,
`TxDownloaderPro_136_2`, `TxDownloaderPro_136_1`, `TxDownloaderPro_136_3`,
`TxDownloaderPro_136_5`.
...
- **Objects read:** `Case`, `Contact`, `Account`, `QuoteLineItems`.
- **Child collections pulled in the same query:** `QuoteLineItems`. A header retrieved without
...
- **Template path roots used:** `Account`, `Contact`, `Quote`, `QuoteLineItems`, `Case`. A
...
- **`$FUN_` value tokens the documents carry:** `$FUN_SPLIT`, `$FUN_SUBSTR`, `$FUN_UNESCAPEXML`,
  `$FUN_STRREPLACE`, `$FUN_ISNULL`. The names are what the templates carry; **no semantics are
...
- **Response fields it reads them from:** `Company`, `CustNum`, `HDCaseNum`, `ConNum`, `CustID`,
  `QuoteNum`, `ShipToNum`. The map is written **source-path first, CRM-field second** (parent
...
- **How many:** 2 community templates, across 1 field-process version.
- **Operations:** 1 carries `IsInsert`, 1 carries `IsUpdate`, 0 carry `IsDelete`. A flag decides
...
- **Field-process versions they belong to:** `TxDownloaderPro_136_3`.
