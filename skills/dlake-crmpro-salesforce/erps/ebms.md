...
description: >-
  What the CRMPro template catalogue carries for an EBMS source pushing into Salesforce: no
  ...
  they write are `Account`, `PricebookEntry`, `Product2`, `Contact`, `EBMS_ShipToAddress__c` and
  more. Use it when deciding whether a shipped template set exists for an EBMS → Salesforce before
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/ebms.md`, and that
...
- Destination objects: `Account`, `PricebookEntry`, `Product2`, `Contact`, `EBMS_ShipToAddress__c`, `User` and 9 custom objects
- Object display names: Contact, EBMS Customer, EBMS CustomerToAccountLookup, EBMS Invoice Detail, EBMS Invoice Header, EBMS Item, EBMS Item Warehouse, EBMS ItemToProductLookup, EBMS Sales Order Detail, EBMS Sales Order Header, EBMS Salesperson, EBMS ShipToAddress, 4 more and 2 further templates
- Template groups: Product, Account, Invoice, Salesorder, Customer Multi Ship Addresses
...
adds what the EBMS → Salesforce template catalogue carries. `dlake-crmpro-salesforce` is the
