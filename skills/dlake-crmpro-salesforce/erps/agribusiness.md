---
name: dlake-crmpro-salesforce/erps/agribusiness
description: >-
  What the CRMPro template catalogue carries for an Agribusiness source pushing into Salesforce:
  no Standard template ships for this pair, and the 18 community templates it does carry are
  ...
  destination objects they write are `Account`, `Order`, `Product2`, `OrderItem`,
  `PricebookEntry`. Use it when deciding whether a shipped template set exists for an Agribusiness
  → Salesforce before standing one up, and what the community set covers. It extends
...
# CRMPro → Salesforce — Agribusiness: what the template catalogue carries
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/agribusiness.md`, and
...
- Destination objects: `Account`, `Order`, `Product2`, `OrderItem`, `PricebookEntry` and 6 custom objects
- Object display names: Account, Get Account, Get Account Ship_To, Get Account_Sold_To, Get Pricebookentry, Get Product2, Get Supplier_Location_Code, Product, SalesOrder, SalesOrder Update, SalesOrderDetails, vw_AGRIBUSINESS_SalesOrder_UpdateKey and 6 further templates
...
adds what the Agribusiness → Salesforce template catalogue carries. `dlake-crmpro-salesforce` is
