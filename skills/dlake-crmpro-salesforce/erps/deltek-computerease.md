---
name: dlake-crmpro-salesforce/erps/deltek-computerease
description: >-
  What the CRMPro template catalogue carries for a Deltek ComputerEase source pushing into
  Salesforce: no Standard template ships for this pair, and the 1 community template it does carry
  are stated as counts, destination objects and template groups only — a community template is
  authored in a tenant, so its names, notes, field mapping and SQL are not published. Use it when
  deciding whether a shipped template set exists for a Deltek ComputerEase → Salesforce before
...
# CRMPro → Salesforce — Deltek ComputerEase: what the template catalogue carries
...
`dlake-crmpro-salesforce/erps/deltek-computerease.md`, and that skill's ERP table is what points
at it. The catalogue ships no Standard template for this pair. Its templates are community
...
The catalogue carries 1 community template for this pair. A community template is authored in a
...
- Templates: 1
- Default operations: insert on 1, update on 1, delete on 1
- Marked circular-sync: 0
- Licence groups they span: 1
- Destination objects: a custom object
- Object display names: Sync Jobs
...
adds what the Deltek ComputerEase → Salesforce template catalogue carries.
