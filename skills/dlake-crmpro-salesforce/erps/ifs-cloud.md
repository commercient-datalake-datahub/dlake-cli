---
name: dlake-crmpro-salesforce/erps/ifs-cloud
description: >-
  What the CRMPro template catalogue carries for an IFS Cloud source pushing into Salesforce: no
  Standard template ships for this pair, and the 87 community templates it does carry are stated
  ...
  they write are `Product2`, `SalesPart__c`, `Opportunity`, `IFS_CloudSupplier__c`,
  `PricebookEntry`. Use it when deciding whether a shipped template set exists for an IFS Cloud →
  Salesforce before standing one up, and what the community set covers. It extends `dlake-crmpro`,
  which covers operating CRMPro generally, and `dlake-crmpro-salesforce`, the destination skill
...
# CRMPro → Salesforce — IFS Cloud: what the template catalogue carries
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/ifs-cloud.md`, and
...
- Destination objects: `Product2`, `SalesPart__c`, `Opportunity`, `IFS_CloudSupplier__c`, `PricebookEntry` and 2 custom objects
- Object display names: SupplierPurchaseParts, Inventory_SalesParts, Sync Products, Sync Supplier, Sync SupplierPurchaseParts, SupplierPurchaseParts_Labor, Get Opportunity, PricebookEntry Standard Create, Get SalesPart and 13 further templates
- Template groups: Product, CRM Opportunity and Line, Pricebook
...
adds what the IFS Cloud → Salesforce template catalogue carries. `dlake-crmpro-salesforce` is the
