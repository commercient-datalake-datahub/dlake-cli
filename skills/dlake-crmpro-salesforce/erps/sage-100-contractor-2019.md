...
description: >-
  ...
  destination objects they write are `Account`, `CommercientSF8__SAGE100CONTRACTOR_Job__c`,
  `Contact`, `CommercientSF8__SAGE100CONTRACTOR_Invoice__c`,
  `CommercientSF8__SAGE100CONTRACTOR_Salesperson__c` and more. Use it when deciding whether a
  shipped template set exists for a Sage 100 Contractor 2019 → Salesforce before standing one up,
...
`dlake-crmpro-salesforce/erps/sage-100-contractor-2019.md`, and that skill's ERP table is what
points at it. The catalogue ships no Standard template for this pair. Its templates are community
templates, authored in a tenant and imported the same way as any other, so what follows is what
that set amounts to — how many templates, which operations they default to, which destination
objects they write and which groups they fall in. A community template's own names, notes, field
mapping and SQL are tenant content and are not published here; read them with `dlake admin
crmpro_templates` against the tenant that holds them. This page grows as the catalogue does.
...
- Destination objects: `Account`, `CommercientSF8__SAGE100CONTRACTOR_Job__c`, `Contact`, `CommercientSF8__SAGE100CONTRACTOR_Invoice__c`, `CommercientSF8__SAGE100CONTRACTOR_Salesperson__c`, `Opportunity`, `CommercientSF8__SAGE100CONTRACTOR_Payment__c`, `CommercientSF8__ACCOUNTMATCHING__c`, `CommercientSF8__CONTACTMATCHING__c`, `CommercientSF8_BudgetLines__c`, `CommercientSF8_CostCodes__c`, `CommercientSF8_PurchaseOrder__c`, `User`, `CommercientSF8__PurchaseOrder__c`, `CommercientSF8_SubContracts__c`, `PricebookEntry`, `Product2`, `SAGE100_Contractor_Invoice_Line__c`, `SAGE100_Contractor_Vendor__c`, `WorkOrder`, 17 more and 17 custom objects
- Object display names: Account, Contact, Job, SalesPerson, Customer Reverse Lookup, Invoice Line, Opportunity, OpportunityToJobLookup, Payment, Invoice, SYNC INVOICE, SYNC INVOICE LINE, 50 more and 19 further templates
- Template groups: Account, Invoice, Product, Salesorder, CRM Opportunity and Line, Opportunity, Purchase Order, CRM Order and Line, Vendor, CRM Vendor, CRM Ownership
...
adds what the Sage 100 Contractor 2019 → Salesforce template catalogue carries.
