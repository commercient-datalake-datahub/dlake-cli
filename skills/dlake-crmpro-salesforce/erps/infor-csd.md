---
name: dlake-crmpro-salesforce/erps/infor-csd
description: >-
  What the CRMPro template catalogue carries for an Infor CSD source pushing into Salesforce: no
  ...
  they write are `Order`, `Account`, `Contact`, `OrderItem`, `Pricebookentry` and more. Use it
  when deciding whether a shipped template set exists for an Infor CSD → Salesforce before
...
# CRMPro → Salesforce — Infor CSD: what the template catalogue carries
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/infor-csd.md`, and
...
- Destination objects: `Order`, `Account`, `Contact`, `OrderItem`, `Pricebookentry`, `Product2`, `SHIPTOMATCHING__c` and 3 custom objects
- Object display names: Account, Contact, Get Existing Pricebookentries, Get Existing Products, INFORCSD Salesperson, Order, Order Item, SYNC SHIPTOMATCHING and 3 further templates
- Template groups: Account, CRM Order and Line, Product, Customer Multi Ship Addresses
...
adds what the Infor CSD → Salesforce template catalogue carries. `dlake-crmpro-salesforce` is the
