---
name: dlake-crmpro-salesforce/erps/erpgenius
description: >-
  What the CRMPro template catalogue carries for an ERPGenius source pushing into Salesforce: no
  Standard template ships for this pair, and the 41 community templates it does carry are stated
  ...
  they write are `Account`, `Opportunity`, `PricebookEntry`, `Product2`, `Contact` and more. Use
  it when deciding whether a shipped template set exists for an ERPGenius → Salesforce before
...
# CRMPro → Salesforce — ERPGenius: what the template catalogue carries
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/erpgenius.md`, and
...
- Destination objects: `Account`, `Opportunity`, `PricebookEntry`, `Product2`, `Contact`, `CommercientSF73__ARTERMS__c`, `Genius_Invoice_Detail__c`, `Genius_Invoice_Header__c`, `Genius_Vendor_Pickup__c`, `User` and 20 custom objects
- Object display names: Account, Contact, Product, Create Standard PriceBook, CustomerToAccount Reverse Lookup, ERPGENIUS Customer, ERPGENIUS CustomerToAccountLookup, ERPGENIUS ItemMaster, ERPGENIUS Order, ERPGENIUS OrderLine, ERPGENIUS PurchaseOrder, ERPGENIUS PurchaseOrderLine, 15 more and 10 further templates
- Template groups: Account, Product, Salesorder, CRM Opportunity and Line, Invoice, Opportunity, Purchase Order, Customer Multi Ship Addresses
...
adds what the ERPGenius → Salesforce template catalogue carries. `dlake-crmpro-salesforce` is the
