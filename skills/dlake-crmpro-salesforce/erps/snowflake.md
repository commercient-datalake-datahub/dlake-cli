---
name: dlake-crmpro-salesforce/erps/snowflake
description: >-
  What the CRMPro template catalogue carries for a Snowflake source pushing into Salesforce: no
  Standard template ships for this pair, and the 36 community templates it does carry are stated
  as counts, destination objects and template groups only — a community template is authored in a
  tenant, so its names, notes, field mapping and SQL are not published. Use it when deciding
  whether a shipped template set exists for a Snowflake → Salesforce before standing one up, and
...
# CRMPro → Salesforce — Snowflake: what the template catalogue carries
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/snowflake.md`, and
...
The catalogue carries 36 community templates for this pair. A community template is authored in a
...
- Templates: 36
- Default operations: insert on 36, update on 36, delete on 36
- Marked circular-sync: 0
- Licence groups they span: 5
- Destination objects: 36 custom objects
- Object display names: 19 further templates
...
adds what the Snowflake → Salesforce template catalogue carries. `dlake-crmpro-salesforce` is the
