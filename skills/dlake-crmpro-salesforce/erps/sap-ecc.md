---
name: dlake-crmpro-salesforce/erps/sap-ecc
description: >-
  What the CRMPro template catalogue carries for a SAP ECC source pushing into Salesforce: no
  ...
  they write are `Account`, `Product2`, `PricebookEntry`, `PriceBookEntry`. Use it when deciding
  whether a shipped template set exists for a SAP ECC → Salesforce before standing one up, and
...
# CRMPro → Salesforce — SAP ECC: what the template catalogue carries
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/sap-ecc.md`, and that
...
- Destination objects: `Account`, `Product2`, `PricebookEntry`, `PriceBookEntry` and 2 custom objects
- Object display names: Account, Product, SYNC CUSTOMER, SYNC CUSTOMERTOACCOUNTLOOKUP, SYNC ITEM, SYNC ITEMTOPRODUCTLOOKUP, SYNC Price Book Entry Create, SYNC Price Book Entry Update
- Template groups: Product, Account
...
adds what the SAP ECC → Salesforce template catalogue carries. `dlake-crmpro-salesforce` is the
