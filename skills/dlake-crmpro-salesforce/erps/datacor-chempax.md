...
description: >-
  ...
  no Standard template ships for this pair, and the 15 community templates it does carry are
  ...
  destination objects they write are `Account`, `Contact`, `Pricebookentry`, `Invoice__c`,
  `Invoice_Line_Item__c` and more. Use it when deciding whether a shipped template set exists for
  a Datacor Chempax → Salesforce before standing one up, and what the community set covers. It
...
this source's own templates set. This page is `dlake-crmpro-salesforce/erps/datacor-chempax.md`,
and that skill's ERP table is what points at it. The catalogue ships no Standard template for this
...
- Destination objects: `Account`, `Contact`, `Pricebookentry`, `Invoice__c`, `Invoice_Line_Item__c`, `Location`, `Order`, `OrderItem`, `Product2`, `ProductInventory__c`, `User`
- Object display names: Account, Contact, Customer Accounts, Invoice, Invoice Line Item, Location, Order, Order Product, Product Inventory, Products, Standard Pricebook Create, Standard Pricebook Update, 2 more and a further template
...
adds what the Datacor Chempax → Salesforce template catalogue carries. `dlake-crmpro-salesforce`
